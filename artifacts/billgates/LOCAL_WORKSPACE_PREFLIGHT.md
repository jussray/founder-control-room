...
- Private storefront CI still returns `steps:null`, so local verification needs a crisp setup gate.
...
Revert the documentation, npm script, preflight script, and static-contract additions. Existing missions remain sandboxed. Existing terminal and evidence rows are immutable and must not be deleted.
