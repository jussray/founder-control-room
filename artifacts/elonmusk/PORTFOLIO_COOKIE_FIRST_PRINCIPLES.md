...
1. Keep one custom cookie: Founder Control Room's server-owned session.
2. Let Shopify own Shopify-required cookies.
3. Keep Se’kret Bip native tokens in SecureStore.
4. Keep all other repositories cookie-free.
5. Reject every new cookie until its server owner, purpose, expiry, security attributes, cache behavior, and deletion path are demonstrated.
