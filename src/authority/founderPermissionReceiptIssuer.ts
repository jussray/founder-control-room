...
function canonicalMergeActionDigest(input: {
  repo: string;
  pullRequestNumber: number;
  headSha: string;
  baseSha: string;
...
export function issueFounderMergePermissionReceipt(
  ...
  const actionTarget = `${source.repo}#${source.pullRequestNumber}`;
  const receipt: AuthorityReceiptV2 & FounderPermissionReceipt = {
    ...
    scope: [`merge:${source.repo}`],
    action: {
      type: 'merge',
      target: actionTarget,
      digest: canonicalMergeActionDigest(source),
    },
    evidence: [
      { ref: `fcr:founder-decision:${source.decisionId}`, class: 'human-approval' },
      { ref: `github:commit:${source.headSha.toLowerCase()}`, class: 'repository' },
      { ref: `github:pull-request:${source.repo}#${source.pullRequestNumber}`, class: 'provider' },
    ],
