...
import { canonicalizeRelayJcsV31 } from '../founder-os-lab/federatedRelay/jcs.js';
import {
...
} from '../founder-os-lab/federatedRelay/crypto.js';
import {
  ...
  RelayV31Error,
} from '../founder-os-lab/federatedRelay/v31-types.js';
import { assertFederatedRelayEnvelopeV31, validateRelayFreshnessV31 } from '../founder-os-lab/federatedRelay/validation.js';
...
export async function validateRelayEndpointBindingsV31(input: {
  ...
  const sourceExists = await deps.sourceEvidence.commitExists({ repository: envelope.source.repository, sha: envelope.source.headSha });
  assert(sourceExists, 'relay_source_commit_unknown');
  const sourceReachableFromClaimedBranch = await deps.sourceEvidence.isCommitReachableFromBranch({ repository: envelope.source.repository, branch: envelope.source.branch, sha: envelope.source.headSha });
...
export async function acceptFederatedRelayV31(input: unknown, deps: RelayAcceptDepsV31): Promise<RelayLedgerAcceptResultV31> {
  ...
  const unsignedReceipt: FederatedAgentRelayReceiptV31Unsigned = {
    ...
    receiver: { member: deps.localIdentity.member, repository: deps.localIdentity.repository, branch: deps.localIdentity.branch, headSha: deps.localIdentity.currentHeadSha },
    sourceHeadSha: envelope.source.headSha,
    targetObservedHeadSha: deps.localIdentity.currentHeadSha,
    sourceCommitEvidence: { repository: envelope.source.repository, branch: envelope.source.branch, headSha: envelope.source.headSha, state: binding.sourceReachableFromClaimedBranch ? 'reachable_at_acceptance' : 'exists_not_currently_reachable', checkedAt: new Date(now).toISOString() },
    predecessorProofCookie: envelope.predecessorProofCookie,
    successorProofCookie,
    evidenceDigest,
    acceptedKeyState: { member: verifiedKey.member, keyId: verifiedKey.keyId, state: verifiedKey.state, validFrom: verifiedKey.validFrom, validUntil: verifiedKey.validUntil },
