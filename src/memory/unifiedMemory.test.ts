...
describe('unified portfolio memory spine', () => {
  it('normalizes Chief Company Brain into verified semantic memory without execution authority', () => {
    ...
    expect(result.record.continuityFingerprint).toMatch(/^memfp:[0-9a-f]{8}$/);
  });
  ...
  it('keeps finance-shaped agent memory metadata-only too', () => {
  ...
  it('maps the memory organs found across the portfolio into one canonical vocabulary', () => {
    const inputs: NativeMemoryObservation[] = [
      {
        sourceSystem: 'founder-control-room', projectSlug: 'founder-control-room', repository: 'jussray/founder-control-room',
      ...
      {
        sourceSystem: 'storyengine', projectSlug: 'l99', repository: 'jussray/StoryEngine',
      ...
      {
        sourceSystem: 'promptos', projectSlug: 'promptos', repository: 'jussray/promptos',
      ...
      {
        sourceSystem: 'think-tank', projectSlug: 'think-tank', repository: 'jussray/THINK-TANK',
      ...
      {
        sourceSystem: 'solcontinuity', projectSlug: 'solcontinuity', repository: 'jussray/solcontinuity',
    ...
    expect(view.summary.verifiedForDecisionSupport).toBe(4);
  });
});
