...
export const FEDERATED_AGENT_MEMBERS_V31 = [
...
export interface RelayMemberIdentityV31 {
...
export interface RelayEvidenceLocatorV31 {
  provider: 'github' | 'cloudflare' | 'supabase' | 'juss-proof';
  ref: string;
}

export interface RelayEvidenceV31 {
  locator: RelayEvidenceLocatorV31;
  state: RelayTruthStateV31;
  sha256?: string;
  proofReceiptId?: string;
}

export type RelayRelationV31 =
  | { type: 'root' }
  | { type: 'reply' | 'revision' | 'reconcile'; parentMessageId: string };

export interface RelayOrderingV31 {
  chainId: string;
  sourceSequence: number;
  chainPosition: number;
  logicalOperationId: string;
  relation: RelayRelationV31;
}

export interface RelaySignatureV31 {
  algorithm: 'Ed25519';
  keyId: string;
...
export interface FederatedAgentRelayEnvelopeV31 {
  ...
  subject: string;
  payload: {
    contentType: RelayContentTypeV31;
    body: string;
    sha256: string;
  };
...
export interface AcceptedKeyStateV31 {
  member: FederatedAgentMemberV31;
  keyId: string;
  state: Exclude<RelayKeyStateV31, 'revoked'>;
  validFrom: string;
...
export interface FederatedAgentRelayReceiptV31Unsigned {
  ...
  acceptedKeyState: AcceptedKeyStateV31;
  acceptedAt: string;
...
export interface FederatedAgentRelayReceiptV31 extends FederatedAgentRelayReceiptV31Unsigned {
  signature: RelaySignatureV31;
}
...
export interface RelaySignatureVerifierV31 {
  verify(input: {
    ...
    keyId: string;
    issuedAt: string;
    canonicalUnsignedBytes: Uint8Array;
    signature: Uint8Array;
  }): Promise<RelayVerifiedKeyV31>;
}

export interface RelayReceiptSignerV31 {
  sign(input: {
    receiver: FederatedAgentMemberV31;
    canonicalReceiptBytes: Uint8Array;
  }): Promise<RelaySignatureV31>;
}

export interface RelaySourceEvidenceResolverV31 {
  commitExists(input: { repository: string; sha: string }): Promise<boolean>;
  isCommitReachableFromBranch(input: { repository: string; branch: string; sha: string }): Promise<boolean>;
}

export interface RelayLocalIdentityV31 {
  member: FederatedAgentMemberV31;
  repository: string;
  branch: string;
...
export interface RelayAcceptDepsV31 {
  localIdentity: RelayLocalIdentityV31;
  sourceEvidence: RelaySourceEvidenceResolverV31;
  signatureVerifier: RelaySignatureVerifierV31;
  receiptSigner: RelayReceiptSignerV31;
  ledger: RelayLedgerV31;
  now?: number;
}
