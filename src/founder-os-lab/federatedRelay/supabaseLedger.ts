...
import type {
  FederatedAgentRelayReceiptV31,
  RelayLedgerAcceptInputV31,
  RelayLedgerAcceptResultV31,
  RelayLedgerV31,
  StoredRelayDeliveryV31,
} from './v31-types.js';

export class RelayTransientErrorV31 extends Error {
  constructor(public readonly code: 'relay_deadlock_retry' | 'relay_serialization_retry') {
    super(code);
    this.name = 'RelayTransientErrorV31';
  }
}
  ...
  return {
    async findByMessageId(messageId: string): Promise<StoredRelayDeliveryV31 | null> {
      const { data, error } = await client
        ...
        .select('message_id,semantic_fingerprint,delivery_fingerprint,receipt,status,superseded_by_message_id')
      ...
      return {
        ...
        semanticFingerprint: data.semantic_fingerprint as string,
        deliveryFingerprint: data.delivery_fingerprint as string,
        receipt: data.receipt as FederatedAgentRelayReceiptV31,
        currentState: data.status as StoredRelayDeliveryV31['currentState'],
        supersededByMessageId: (data.superseded_by_message_id as string | null) ?? null,
      };
    },
    ...
    async accept(input: RelayLedgerAcceptInputV31): Promise<RelayLedgerAcceptResultV31> {
      ...
      const { data, error } = await client.rpc('federated_relay_accept_v31', {
        ...
        p_parent_message_id: relationParentMessageId(input),
        ...
        p_accepted_key_state: input.receipt.acceptedKeyState,
      ...
      if (!row || (row.outcome !== 'accepted' && row.outcome !== 'duplicate') || !row.stored_receipt) {
        throw new Error('relay_database_result_invalid');
      }

      return {
        outcome: row.outcome,
        receipt: row.stored_receipt as FederatedAgentRelayReceiptV31,
        currentState: row.current_state as RelayLedgerAcceptResultV31['currentState'],
        supersededByMessageId: (row.superseded_by_message_id as string | null) ?? null,
      };
    },
  };
}
