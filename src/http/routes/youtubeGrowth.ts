...
function optionalRecord(value: unknown): boolean {
  return value === undefined || record(value) !== null;
}
...
function validInputShape(value: unknown): value is YouTubeGrowthLoopInput {
  ...
  if (!optionalRecord(candidate.diagnosticThresholds)) return false;
  if (!optionalRecord(candidate.continuity)) return false;
  if (!optionalRecord(candidate.targets)) return false;
...
youtubeGrowthRouter.post('/evaluate', (req: FounderRequest, res) => {
  if (!validInputShape(req.body)) {
    return res.status(400).json({
      ...
      contract: YOUTUBE_GROWTH_EVALUATION_CONTRACT,
      reasons: [
        'A typed 90-day growth evaluation payload is required; malformed experiments or measurements are rejected before core evaluation.',
      ],
    });
  }
