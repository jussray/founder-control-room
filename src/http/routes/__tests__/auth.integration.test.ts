...
describe('GET /auth/callback', () => {
  ...
  it('preserves an existing browser session when verifyOtp fails', async () => { mockVerifyOtp.mockResolvedValue({ data: {}, error: { message: 'expired' } }); const res = await request(buildApp()).get('/auth/callback').query({ token_hash: 'bad' }).set('Cookie', `__Host-fcr_session=${VALID_OPAQUE_COOKIE}`); expect(res.status).toBe(401); expect(res.headers['set-cookie']).toBeUndefined(); });
  it('preserves an existing browser session when a newly verified user is not on the allowlist', async () => { mockVerifyOtp.mockResolvedValue({ data: { session: SESSION, user: { id: FOUNDER_USER_ID, email: 'stranger@example.com' } }, error: null }); setAllowlist(false); const res = await request(buildApp()).get('/auth/callback').query({ token_hash: 'good' }).set('Cookie', `__Host-fcr_session=${VALID_OPAQUE_COOKIE}`); expect(res.status).toBe(401); expect(res.headers['set-cookie']).toBeUndefined(); });
...
describe('POST /auth/session', () => {
  it('rejects missing implicit-flow credentials without clearing an existing browser session', async () => { const res = await request(buildApp()).post('/auth/session').set('Cookie', `__Host-fcr_session=${VALID_OPAQUE_COOKIE}`).send({}); expect(res.status).toBe(400); expect(res.headers['set-cookie']).toBeUndefined(); expect(mockSetSession).not.toHaveBeenCalled(); });
  it('rejects invalid implicit-flow credentials without clearing an existing browser session', async () => { mockSetSession.mockResolvedValue({ data: {}, error: { message: 'expired' } }); const res = await request(buildApp()).post('/auth/session').set('Cookie', `__Host-fcr_session=${VALID_OPAQUE_COOKIE}`).send({ access_token: 'bad-at', refresh_token: 'bad-rt' }); expect(res.status).toBe(401); expect(res.headers['set-cookie']).toBeUndefined(); });
