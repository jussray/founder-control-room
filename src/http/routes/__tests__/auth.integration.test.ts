...
describe('GET /auth/callback', () => {
  ...
  it('binds the separately verified user into opaque session persistence and redirects', async () => { mockVerifyOtp.mockResolvedValue({ data: { session: SESSION, user: VERIFIED_USER }, error: null }); setAllowlist(true); const res = await request(buildApp()).get('/auth/callback').query({ token_hash: 'good' }).redirects(0); expect(res.status).toBe(303); expectSessionCookie(res); const location = String(res.headers.location); expect(location.startsWith('/control-room/#')).toBe(true); const fragment = new URLSearchParams(location.split('#')[1]); expect(fragment.get('access_token')).toBe(SESSION.access_token); expect(fragment.get('refresh_token')).toBe(SESSION.refresh_token); expect(fragment.get('email')).toBe(FOUNDER_EMAIL); });
});
