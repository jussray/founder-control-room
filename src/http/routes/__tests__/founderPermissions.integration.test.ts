...
vi.mock('../../../lib/supabaseClient.js', () => ({
  supabase: {
    from: vi.fn((table: string) => {
      ...
      if (table !== 'founder_permission_requests') {
      ...
      const chain: any = {
        ...
        maybeSingle: async () => {
          ...
          if (operation === 'update') {
            if (!existing || (filters.has('status') && existing.status !== filters.get('status'))) {
        ...
        single: async () => {
          ...
          const now = '2026-08-19T19:30:00.000Z';
          const inserted = {
            ...payload,
            decision: null,
            decision_hash: null,
...
describe('founder permission broker HTTP contract', () => {
  ...
  it('keeps a request non-authoritative until the founder explicitly approves it', async () => {
    ...
    const created = await request(app)
      ...
      .send({
        requestId: 'permission:ask-ray-001',
        requestedBySurface: 'chatgpt',
        proposal,
        note: 'Approve this exact candidate?',
      });
  ...
  it('fails closed when the same request id is reused for another proposal', async () => {
    ...
    const first = await request(app)
      ...
      .set('Authorization', bearer)
      .send({
        requestId: 'permission:ask-ray-002',
    ...
    const conflicting = await request(app)
      ...
      .send({
        requestId: 'permission:ask-ray-002',
        requestedBySurface: 'claude',
        proposal: { ...proposal, expectedHeadSha: 'd'.repeat(40) },
      });
  ...
  it('does not let a later approval overwrite an existing rejection', async () => {
    ...
    await request(app)
      ...
      .set('Authorization', bearer)
      .send({
        requestId: 'permission:ask-ray-003',
