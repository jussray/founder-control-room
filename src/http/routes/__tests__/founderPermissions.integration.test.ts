      ...
      const chain: any = {
        ...
        single: async () => {
          ...
          const inserted = {
            ...
            requested_at: '2026-08-19T19:30:00.000Z',
            decided_at: null,
            consumed_at: null,
          };
...
describe('founder permission broker HTTP contract', () => {
  beforeEach(() => {
    ...
    mockGetUser.mockResolvedValue({
      data: { user: { id: '11111111-1111-4111-8111-111111111111', email: 'founder@example.com' } },
      error: null,
    });
  });
  ...
  it('keeps a request non-authoritative until the founder explicitly approves it', async () => {
    ...
    const created = await request(app)
      .post('/mcp/founder-permissions/requests')
      .set('Authorization', bearer)
      .send({ requestId: 'permission:ask-ray-001', requestedBySurface: 'chatgpt', proposal, note: 'Approve this exact candidate?' });
    ...
    const approved = await request(app)
      .post('/mcp/founder-permissions/requests/permission:ask-ray-001/decision')
      .set('Authorization', bearer)
  ...
  it('fails closed when the same request id is reused for another proposal', async () => {
    ...
    const first = await request(app)
      .post('/mcp/founder-permissions/requests')
      .set('Authorization', bearer)
      .send({ requestId: 'permission:ask-ray-002', requestedBySurface: 'claude', proposal });
    expect(first.status).toBe(201);

    const conflicting = await request(app)
      .post('/mcp/founder-permissions/requests')
      .set('Authorization', bearer)
      .send({ requestId: 'permission:ask-ray-002', requestedBySurface: 'claude', proposal: { ...proposal, expectedHeadSha: 'd'.repeat(40) } });
  ...
  it('does not let a later approval overwrite an existing rejection', async () => {
    ...
    await request(app)
      .post('/mcp/founder-permissions/requests')
      .set('Authorization', bearer)
      .send({ requestId: 'permission:ask-ray-003', requestedBySurface: 'perplexity', proposal });

    const rejected = await request(app)
      .post('/mcp/founder-permissions/requests/permission:ask-ray-003/decision')
      .set('Authorization', bearer)
