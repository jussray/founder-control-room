...
founderPermissionsRouter.post('/requests', async (req: FounderRequest, res) => {
  ...
  let request: FounderPermissionRequest;
  try {
    request = createFounderPermissionRequest({
  ...
  if (existing) {
    ...
    if (text(row.request_hash).toLowerCase() !== request.requestHash) {
...
founderPermissionsRouter.post('/requests/:requestId/decision', async (req: FounderRequest, res) => {
  ...
  let resolution;
  try {
    resolution = resolveFounderPermissionRequest({ request, decisionSurface, decision });
  } catch (error) {
