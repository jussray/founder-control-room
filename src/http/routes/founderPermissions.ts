...
function proposalFrom(value: unknown): FounderControlProposalBinding | null {
  ...
  return {
    ...
    proposalHash: text(value.proposalHash),
    projectSlug: text(value.projectSlug),
    actionType: text(value.actionType),
    expectedHeadSha: text(value.expectedHeadSha) || null,
...
function projection(row: JsonRecord) {
  ...
  return {
    ...
    status,
    requestedBySurface: surfaceFrom(row.requested_by_surface),
    requestHash: text(row.request_hash) || null,
    proposal: isRecord(row.proposal) ? row.proposal : null,
    note: text(row.note) || null,
    decision: isRecord(row.decision) ? row.decision : null,
    decisionHash: text(row.decision_hash) || null,
    decisionSurface: surfaceFrom(row.decision_surface),
    requestedAt: text(row.requested_at) || null,
    decidedAt: text(row.decided_at) || null,
    consumedAt: text(row.consumed_at) || null,
    founderPermissionSatisfied: status === 'approved',
    // Deliberately unknown here. Independent review belongs to repository review authority.
    independentReviewSatisfied: null,
  };
}

export const founderPermissionsRouter = Router();
founderPermissionsRouter.use(requireFounder);
...
founderPermissionsRouter.get('/requests', async (req: FounderRequest, res) => {
  ...
  if (req.query.status !== undefined && !requestedStatus) {
    return res.status(400).json({ error: 'unsupported founder permission status' });
  }
...
founderPermissionsRouter.post('/requests', async (req: FounderRequest, res) => {
  ...
  if (!body || !requestedBySurface || !proposal) {
    return res.status(400).json({ error: 'requestId, requestedBySurface, and proposal are required.' });
  }
  ...
  try {
    permissionRequest = createFounderPermissionRequest({
      ...
      requestedBySurface,
      proposal,
      note: body.note === null || body.note === undefined ? null : text(body.note),
    });
  } catch (error) {
  ...
  const { data: existing, error: lookupError } = await supabase
    .from('founder_permission_requests')
    .select('request_id,requested_by_surface,request_hash,proposal,note,status,decision,decision_hash,decision_surface,requested_at,decided_at,consumed_at')
    .eq('request_id', permissionRequest.requestId)
    .maybeSingle();
  ...
  if (existing) {
    ...
    if (text(row.request_hash).toLowerCase() !== permissionRequest.requestHash) {
      return res.status(409).json({ error: 'requestId is already bound to a different proposal.', code: 'FOUNDER_PERMISSION_SCOPE_MISMATCH' });
    }
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      request_contract: permissionRequest.contract,
      requested_by_surface: permissionRequest.requestedBySurface,
      request_hash: permissionRequest.requestHash,
      proposal: permissionRequest.proposal,
      note: permissionRequest.note,
      status: 'pending',
    })
    .select('request_id,requested_by_surface,request_hash,proposal,note,status,decision,decision_hash,decision_surface,requested_at,decided_at,consumed_at')
    .single();
...
founderPermissionsRouter.post('/requests/:requestId/decision', async (req: FounderRequest, res) => {
  ...
  if (!requestId || !body || !decision || !decisionSurface) {
    return res.status(400).json({ error: 'requestId, decision, and supported surface are required.' });
  }
  ...
  let resolution;
  try {
    resolution = resolveFounderPermissionRequest({ request: permissionRequest, decisionSurface, decision });
  } catch (error) {
    return res.status(409).json({ error: error instanceof Error ? error.message : String(error), code: 'FOUNDER_PERMISSION_REQUEST_INVALID' });
  }

  const currentStatus = statusFrom(row.status) ?? 'pending';
  if (currentStatus !== 'pending') {
    if (currentStatus === resolution.status && text(row.decision_hash).toLowerCase() === resolution.decision.decisionHash) {
      return res.json({ idempotent: true, ...projection(row) });
    }
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      decision_surface: resolution.decision.surface,
      founder_user_id: req.founder!.userId,
      founder_email: req.founder!.email,
  ...
  if (!data) {
    return res.status(409).json({ error: 'Founder permission request changed before decision could be recorded.', code: 'FOUNDER_PERMISSION_DECISION_RACE' });
  }
  return res.json(projection(data as JsonRecord));
});
