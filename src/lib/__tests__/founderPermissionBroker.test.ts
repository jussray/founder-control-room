...
describe('Founder Permission Broker', () => {
  ...
  it('turns explicit founder approval into the canonical founder decision receipt only', () => {
    const request = createFounderPermissionRequest({
    ...
    const resolution = resolveFounderPermissionRequest({
      request,
      decisionSurface: 'chatgpt',
      decision: 'approved',
    });
  ...
  it('rejects a tampered request before issuing a decision receipt', () => {
    const request = createFounderPermissionRequest({
    ...
    expect(() => resolveFounderPermissionRequest({
      request: {
        ...request,
        proposal: { ...request.proposal, expectedHeadSha: 'd'.repeat(40) },
      },
