...
describe('founder content analytics CSV ingestion', () => {
  it('binds a safe CSV import to account, source, historical provenance, units, and explicit audience segments', () => {
    ...
    expect(receipt.metric_schema.engagements).toEqual({ unit: 'count', nullable: true });
    expect(receipt.metric_schema.gross_new_followers).toEqual({ unit: 'count', nullable: true });
    expect(receipt.metric_schema.audience_share).toEqual({ unit: 'ratio_0_to_1', nullable: false });
    expect(receipt.metric_schema.audience_delta).toEqual({ unit: 'percentage_points', nullable: true });
  ...
  it('is idempotent for identical bytes and metadata and changes identity when provenance changes', () => {
  ...
  it('rejects duplicate snapshot provenance hidden behind a new snapshot id', () => {
    const duplicateSnapshot = safeCsv.replaceAll(
      'current-2026-09-03',
      'current-duplicate-id',
    );
    const combined = `${safeCsv.trim()}\n${duplicateSnapshot.split('\n').slice(5).join('\n').trim()}\n`;

    expect(() => parseFounderContentAnalyticsCsv(combined, metadata))
      .toThrow(/duplicate the same capture provenance/);
  });

  it('requires actual UTC timestamps rather than accepting date-only provenance', () => {
    expect(() => parseFounderContentAnalyticsCsv(safeCsv, {
      ...metadata,
      generated_at: '2026-09-04',
    })).toThrow(/generated_at must be an ISO UTC timestamp/);

    const dateOnlyCapture = safeCsv.replaceAll('2026-09-03T23:00:00.000Z', '2026-09-03');
    expect(() => parseFounderContentAnalyticsCsv(dateOnlyCapture, metadata))
      .toThrow(/captured_at must be an ISO UTC timestamp/);
  });

  it('rejects daily rows outside the declared snapshot window', () => {
