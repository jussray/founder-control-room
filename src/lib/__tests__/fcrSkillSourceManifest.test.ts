...
} from '../fcrSkillSourceManifest.js';
...
describe('FCR skill source manifest', () => {
  ...
  it('preserves folded descriptions from founder-owned YAML frontmatter', async () => {
    const manifest = await discoverFcrSkillSources(REPOSITORY_SKILLS_ROOT);
    const fleet = manifest.entries.find((entry) => entry.id === 'skill:control-room-cloudflare-agent-fleet');

    expect(fleet).toBeDefined();
    expect(fleet?.description).toContain('Shared Cloudflare operating contract');
    expect(fleet?.description).not.toBe('>');
  });
