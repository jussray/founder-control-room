...
export function mergeExistingRulesetSecurity({
  ...
  if (blockForcePushes || existingByType.has("non_fast_forward")) {
    nextRules.push(structuredClone(existingByType.get("non_fast_forward") ?? requestedByType.get("non_fast_forward") ?? { type: "non_fast_forward" }));
  }

  if (blockDeletion || existingByType.has("deletion")) {
    nextRules.push(structuredClone(existingByType.get("deletion") ?? requestedByType.get("deletion") ?? { type: "deletion" }));
  }
