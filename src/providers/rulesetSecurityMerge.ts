...
/**
 * Existing GitHub rulesets can carry provider security semantics that the
 * generic RulesetConfig does not model (CodeQL, Copilot review, extra review
 ...
 */
export function mergeExistingRulesetSecurity({
  existingRules,
  requestedRules,
  ...
  blockForcePushes,
  blockDeletion,
}: MergeExistingRulesetSecurityInput): RulesetRuleLike[] {
  ...
  const nextRules: RulesetRuleLike[] = existingRules
    ...
    nextRules.push({
      ...
      parameters: {
        ...
        required_approving_review_count: Math.max(
          reviewCount(existingParameters),
          reviewCount(requestedParameters),
        ),
        dismiss_stale_reviews_on_push: securityBoolean(
          existingParameters,
          requestedParameters,
        ...
        required_review_thread_resolution: securityBoolean(
          existingParameters,
          requestedParameters,
  ...
  if (requiredStatusCheckNames.length > 0) {
    ...
    const requestedRule = requestedByType.get("required_status_checks") ?? {
    ...
    const existingChecks = new Map(
      checkEntries(existingParameters)
        .map((entry) => [contextOf(entry), entry] as const)
        .filter(([context]) => context.length > 0),
    );
    const requestedChecks = new Map(
      checkEntries(requestedParameters)
        .map((entry) => [contextOf(entry), entry] as const)
        .filter(([context]) => context.length > 0),
    );
    ...
    nextRules.push({
      ...
      parameters: {
        ...
        strict_required_status_checks_policy:
          existingParameters["strict_required_status_checks_policy"] === true
          || requestedParameters["strict_required_status_checks_policy"] === true,
        required_status_checks: requiredStatusCheckNames.map((context) =>
          structuredClone(existingChecks.get(context) ?? requestedChecks.get(context) ?? { context }),
        ),
      },
    });
  }
