...
export function requestedRefsRemainingExcluded(existingExcludes: string[], requested: string[]): string[] {
  const excluded = new Set(existingExcludes.map(normalizeBranchRef));
  return requested.map(normalizeBranchRef).filter((ref) => excluded.has(ref));
}
...
export class SecurityPreservingGitHubProvider extends GitHubProvider {
  ...
  override async applyBranchRuleset(projectId: string, config: RulesetConfig): Promise<RulesetResult> {
    ...
    const remainingExcluded = requestedRefsRemainingExcluded(currentExcludes, config.targetRefs);
    if (remainingExcluded.length > 0) {
      throw new Error(
        `SecurityPreservingGitHubProvider: requested target refs remain excluded by the existing provider ruleset: ${remainingExcluded.join(", ")}`,
      );
    }
    ...
    throw new Error(
      "SecurityPreservingGitHubProvider: existing non-FCR ruleset mutation is disabled until provider-supported atomic concurrency or an equivalent reconciliation contract can prove monotonic security across the write boundary",
    );
  }
}
