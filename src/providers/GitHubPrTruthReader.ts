...
function sameMutablePrIdentity(left: GitHubPrAuditContext, right: GitHubPrAuditContext): boolean {
  return left.number === right.number
    ...
    && left.baseShaSnapshot === right.baseShaSnapshot
...
function checkSnapshot(values: readonly NormalizedCheck[]): string {
...
function requiredCheckSnapshot(value: RequiredCheckDiscovery): string {
  return JSON.stringify({
    state: value.state,
    source: value.source,
    requiredChecks: [...value.requiredChecks].sort((left, right) =>
      `${left.kind}:${left.context}:${left.appId ?? ''}`.localeCompare(`${right.kind}:${right.context}:${right.appId ?? ''}`)),
    findings: [...value.findings].sort(),
  });
}

function reviewSnapshot(values: readonly ReviewSignal[]): string {
  return JSON.stringify(values
    .map((item) => ({
      id: item.id,
...
export class GitHubPrTruthReader implements GitHubPrTruthReaderLike {
  ...
  private async statuses(token: string, headSha: string): Promise<{ observations: NormalizedCheck[]; truncated: boolean }> {
    const observedAt = this.now().toISOString();
    const latestByContext = new Map<string, StatusCandidate>();
    let exhausted = false;
    for (let page = 1; page <= MAX_PAGES; page += 1) {
      const value = await requestJson(
        this.fetchFn,
      ...
      for (const item of value) {
        ...
        if (!context) continue;
        const updatedAt = text(item.updated_at) || text(item.created_at);
        const id = numberOrZero(item.id);
        ...
        if (!current || updatedAt > current.updatedAt || (updatedAt === current.updatedAt && id > current.id)) {
          latestByContext.set(context, candidate);
        }
      }
    ...
    return {
      observations: [...latestByContext.values()].map((candidate) => candidate.observation),
      truncated: !exhausted,
    };
  }
  ...
  async readAuditEvidence(pullNumber: number): Promise<GitHubPrTruthEvidence> {
    ...
    const initialCheckRuns = await this.checks(token, initialContext.headSha);
    const initialStatuses = await this.statuses(token, initialContext.headSha);
    const initialChecks = [...initialCheckRuns.observations, ...initialStatuses.observations];
    if (initialCheckRuns.truncated || initialStatuses.truncated) kernelFindings.push('collection_truncated');

    const initialRequiredChecks = await this.requiredChecks(token, initialContext.baseRef, initialChecks);
    const initialReviewPacket = await this.reviews(token, pullNumber);
    if (initialReviewPacket.truncated) transportFindings.push('review_collection_truncated');
    ...
    if (finalContext.baseRef !== FCR_TARGET_BRANCH) transportFindings.push('target_branch_not_main');
    ...
    const finalCheckRuns = await this.checks(token, finalContext.headSha);
    const finalStatuses = await this.statuses(token, finalContext.headSha);
    const finalChecks = [...finalCheckRuns.observations, ...finalStatuses.observations];
    if (finalCheckRuns.truncated || finalStatuses.truncated) kernelFindings.push('collection_truncated');
    const finalRequiredChecks = await this.requiredChecks(token, finalContext.baseRef, finalChecks);
    const finalReviewPacket = await this.reviews(token, pullNumber);
    if (finalReviewPacket.truncated) transportFindings.push('review_collection_truncated');

    if (checkSnapshot(initialChecks) !== checkSnapshot(finalChecks)) {
      kernelFindings.push('ci_evidence_changed_during_collection');
    }
    if (requiredCheckSnapshot(initialRequiredChecks) !== requiredCheckSnapshot(finalRequiredChecks)) {
      kernelFindings.push('required_check_policy_changed_during_collection');
    }
    if (
      initialReviewPacket.truncated !== finalReviewPacket.truncated
      || reviewSnapshot(initialReviewPacket.reviews) !== reviewSnapshot(finalReviewPacket.reviews)
    ) {
      kernelFindings.push('review_evidence_changed_during_collection');
    }
    ...
    return {
      ...
      checks: finalChecks,
      reviews: finalReviewPacket.reviews,
      diff,
