...
describe("mergeExistingRulesetSecurity", () => {
  it("narrows required contexts without weakening existing provider security", () => {
    ...
    const requestedRules = [
      { type: "pull_request", parameters: { required_approving_review_count: 1, dismiss_stale_reviews_on_push: false, require_last_push_approval: false, required_review_thread_resolution: true, require_code_owner_review: false } },
      { type: "required_status_checks", parameters: { strict_required_status_checks_policy: true, do_not_enforce_on_create: false, required_status_checks: [{ context: "Typecheck" }, { context: "Unit Tests" }] } },
      { type: "non_fast_forward" },
      { type: "deletion" },
    ];
  ...
  it("preserves existing managed protections when a generic caller requests none", () => {
    const existingRules = [
      { type: "deletion" },
      { type: "non_fast_forward" },
      { type: "pull_request", parameters: { required_approving_review_count: 2, dismiss_stale_reviews_on_push: true, require_code_owner_review: true, require_last_push_approval: true, required_review_thread_resolution: true } },
    ];
  ...
  it("leaves creation semantics outside the helper", () => {
    expect(mergeExistingRulesetSecurity({
      ...
      { type: "required_status_checks", parameters: { required_status_checks: [{ context: "Typecheck" }], strict_required_status_checks_policy: false } },
    ]);
  });
});
