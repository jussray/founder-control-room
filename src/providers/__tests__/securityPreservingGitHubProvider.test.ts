...
describe("SecurityPreservingGitHubProvider", () => {
  beforeEach(() => {
    ...
    mockGetRepoRulesets.mockResolvedValue({
      data: [{ id: 21261587, name: "governance boundary" }],
    });
    ...
    mockUpdateRepoRuleset.mockResolvedValue({
      data: { id: 21261587, name: "governance boundary", enforcement: "active" },
    });
  });
  ...
  it("updates an existing ruleset in place while preserving stronger security", async () => {
    ...
    const pullRequest = call.rules.find((rule: { type: string }) => rule.type === "pull_request");
