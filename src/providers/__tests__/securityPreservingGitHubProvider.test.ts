...
describe("SecurityPreservingGitHubProvider", () => {
  ...
  it("also blocks an explicit empty bypass clear until the existing-ruleset write is concurrency-safe", async () => {
    ...
    await expect(provider().applyBranchRuleset(PROJECT_ID, config({ bypassActors: [] }))).rejects.toThrow(
      "existing non-FCR ruleset updates are blocked until a concurrency-safe provider reconciliation contract exists",
    );
    expect(mockUpdateRepoRuleset).not.toHaveBeenCalled();
  });
  ...
  it("keeps non-empty bypass replacement fail-closed before the concurrency gate", async () => {
    mockGetRepoRuleset.mockResolvedValueOnce({ data: {
      id: 21261587,
      name: "governance boundary",
      enforcement: "active",
