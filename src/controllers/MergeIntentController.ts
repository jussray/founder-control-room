...
export class MergeIntentController extends BaseController {
  ...
  protected async reconcile(req: ReconcileRequest): Promise<ReconcileResult> {
    ...
    // These states are deliberately sticky. Reconciliation can discover and
    // preserve revocation, but only a new founder approval revision can clear it.
    ...
    if (intent.state === 'blocked') {
      return result('blocked', 'Merge intent is blocked by a durable authority failure; explicit reapproval/reconciliation is required.', intent, 'blocked');
    }
    ...
    const { data: proof, error: proofError } = await supabase
      ...
      .select('id, mission_id, project_id, gate_id, status, approved_by, ran_at')
    ...
    if (
      ...
      return this.transition(
        intent,
        { state: 'needs_review', stale_reason: 'approval proof identity/status no longer matches intent' },
        'blocked',
    ...
    if (
      ...
      return this.transition(
        intent,
        { state: 'needs_review', stale_reason: 'mission policy snapshot drifted from approved intent' },
        'blocked',
    ...
    if (lower(project.repo_identifier) !== lower(intent.repository)) {
      return this.transition(
        intent,
        { state: 'needs_review', stale_reason: 'repository identity changed after approval' },
        'blocked',
    ...
    try {
      ...
      return result(
        'retry',
        `Merge intent provider is unavailable without changing approval state: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    if (typeof provider.getPullRequestReviewContext !== 'function') {
      return this.transition(
        intent,
        { state: 'blocked', stale_reason: 'provider cannot read exact pull request context' },
        'blocked',
    ...
    try {
      ...
      return result(
        'retry',
        `Merge intent PR context is temporarily unavailable without changing approval state: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    let diff: Diff;
    try {
      diff = await provider.compare(project.slug, intent.approved_base_sha, intent.approved_head_sha);
    } catch (error) {
      return result(
        'retry',
        `Merge intent provider diff is temporarily unavailable without changing approval state: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    try {
      ...
      return this.transition(
        ...
        {
          state: 'blocked',
          stale_reason: 'provider diff completeness could not be proven',
          last_observed_base_sha: currentBase,
          last_observed_head_sha: currentHead,
          last_observed_diff_hash: null,
        },
