...
describe('external FCR operator relay authority boundary', () => {
  ...
  it('records a blocked handoff as read-only and not as an external provider call', async () => {
    const relayOperator = vi.fn(async (input: RelayCall) => ({
      request: input,
      response: {
        fromOperator: input.toOperator,
        ...
        answer: 'Provider API is unavailable; an interactive-browser handoff is required.',
        evidenceRefs: [],
        unresolved: ['relay_transport:interactive_browser'],
      },
    }));
    const recordEvidence = vi.fn(async (input: { toolName: string }) => (
      receipt(input.toolName as 'fcr_relay_operator')
    ...
    const result = await execute({
      ...
      arguments: {
        ...
        capability: 'review',
        goal: 'Review without substituting another provider.',
        contextSummary: 'The requested provider has no direct API configuration.',
        sensitivity: 'internal',
      },
      allowedProjects: new Set(['founder-control-room']),
      identity: {
      ...
      requestId: 'relay-request-blocked',
    });
