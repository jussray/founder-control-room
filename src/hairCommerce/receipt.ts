...
function githubEvidenceUrl(
  ...
  const expectedPath = `/${sourceRepo}/commit/${exactCommitSha}`.toLowerCase();
  if (
    ...
    throw new HairCommerceReceiptError('evidence_url_mismatch');
  }
...
export function validateHairCommerceReceipt(input: unknown): HairCommerceReceipt {
  ...
  return {
    ...
    exactCommitSha,
    evidenceUrl: githubEvidenceUrl(input.evidenceUrl, SOURCE_REPO, exactCommitSha),
  };
}
