...
export const capabilities = [
  ...
  {
    id: 'local-playwright-browser-v1', kind: 'Automation', category: 'integrations', score: 97,
    summary: 'Drive approved browser work locally with Playwright instead of a metered remote browser provider.',
    purpose: 'Give FCR a zero-provider-fee browser execution lane for navigation, forms, screenshots, and browser evidence while preserving the shared authority, approval, receipt, and outcome-verification spine.',
    inputs: [
      ['intent', 'text', 'Bounded founder outcome'],
      ['url', 'https URL', 'Exact target page'],
      ['proposalId', 'string', 'Exact approved proposal identity for any mutation'],
      ['authorityRevision', 'string', 'Live authority revision rechecked before mutation'],
      ['payloadSnapshotRef', 'evidence ref', 'Exact approved form payload when data entry is required'],
    ],
    environment: [
      'FCR shared capability runtime remains the control plane',
      'Repository-pinned Playwright and local Chromium',
      'LOCAL_NO_PROVIDER_FEE; no browser wallet or remote browser dependency',
    ],
    proof: [
      'Browser preflight identifies local-playwright and LOCAL_NO_PROVIDER_FEE',
      'Mutations require exact proposal-bound founder approval plus a live authority recheck',
      'Playwright screenshot and final-page evidence are captured for browser runs',
      'Provider acceptance and verified founder outcome remain separate truth states',
      'Continuity fingerprints and proof cookies update or invalidate bidirectionally from browser receipts without becoming authority',
    ],
    risk: 'Local browser automation only. It does not bypass login, CAPTCHA, rate limits, or target-site policy; it does not grant itself approval; and it does not treat a click or HTTP success as verified outcome without confirmation evidence.',
    implementation: `browserExecutor=local-playwright; costClass=LOCAL_NO_PROVIDER_FEE; runtime=repository-playwright;\n// Use the existing FCR authority spine. For writes, bind approval to proposalId,\n// recheck authority at the commit boundary, execute in local Chromium, capture\n// screenshot/final-page receipt, verify outcome, then update continuity markers.`,
  },
  {
  ...
  },
  {
    ...
    summary: 'Bind external application submissions to an exact approved payload, local Playwright readiness, exact uploaded artifacts, and a captured confirmation receipt.',
    purpose: 'Prevent FCR from losing approved form answers, silently reconstructing missing facts, discovering browser/auth blockers only after work begins, or claiming submission without provider confirmation evidence.',
    inputs: [
      ...
      ['providerReadiness', 'preflight', 'Local Playwright, target authentication, and execution readiness checked before form mutation'],
    ],
    environment: ['FCR remains the control plane', 'Local Playwright is the browser execution surface', 'No paid browser wallet or remote browser provider is required', 'No missing field may be invented from adjacent documents or memory'],
    proof: [
      ...
      'Local Playwright readiness is checked before form mutation begins',
    ...
    risk: 'Contract and validation only. It does not create provider credentials, require a browser wallet, bypass login or CAPTCHA, invent missing application answers, or independently grant submission authority.',
    implementation: `validateExternalApplicationSubmission(bundle) -> { ready, blockers };\n// ready=false on missing required fields, missing payload snapshot, hash drift, missing uploads,\n// non-local browser execution, paid/remote browser execution, or local Playwright readiness failure.\nvalidateExternalApplicationConfirmation(receipt, approvedBundle) -> { ready, blockers };\n// completion is allowed only when confirmation evidence binds to the same approved payload and artifacts.`,
  },
  {
