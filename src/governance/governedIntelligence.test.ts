...
function authorizationRequest(overrides: Partial<Parameters<typeof authorizationSupportsAction>[1]> = {}) {
  return {
    requiredScope: 'deploy',
    risk: 'consequential' as const,
    intentId: 'intent-current',
    intentHash: 'current-intent-v1',
    intentSource: 'current_user' as const,
    proposalId: 'proposal-1',
    proposalHash: PROPOSAL_HASH,
    actionHash: ACTION_HASH,
    exactVersion: 'abc123',
    replayState: 'unused' as const,
    now: NOW,
    ...overrides,
  };
}
...
describe('temporal authority', () => {
  it('lets fresh authenticated Current You outrank a FutureYou projection', () => {
    const resolved = resolveTemporalIntent([
      intent({ id: 'future', source: 'future_you', intentHash: 'future-projection', issuedAt: '2026-08-17T03:25:00.000Z', authenticated: false }),
      intent(),
    ], 'deploy', NOW);
    expect(resolved).toMatchObject({ mode: 'authoritative', selected: { id: 'intent-current', source: 'current_user' } });
  });

  it('does not let advisory FutureYou suppress Current You through supersedes', () => {
    const resolved = resolveTemporalIntent([
      intent(),
      intent({ id: 'future', source: 'future_you', authenticated: false, intentHash: 'future-projection', issuedAt: '2026-08-17T03:25:00.000Z', supersedes: ['intent-current'] }),
    ], 'deploy', NOW);
  ...
  it('does not let delegated authority supersede stronger Current You authority', () => {
    const resolved = resolveTemporalIntent([
      intent(),
      intent({ id: 'delegate', source: 'delegated', intentHash: 'delegate-plan', issuedAt: '2026-08-17T03:25:00.000Z', supersedes: ['intent-current'] }),
    ], 'deploy', NOW);
    expect(resolved.mode).toBe('authoritative');
    expect(resolved.selected?.id).toBe('intent-current');
  });
  ...
  it('fails closed when expiry metadata is malformed', () => {
    expect(resolveTemporalIntent([intent({ expiresAt: 'not-a-time' })], 'deploy', NOW).mode).toBe('missing');
  });

  it('does not let revoked Current You intent remain active authority', () => {
    const resolved = resolveTemporalIntent([
      intent({ revokedAt: '2026-08-17T03:21:00.000Z' }),
      intent({ id: 'history', source: 'historical_user', intentHash: 'older-plan', issuedAt: '2026-08-16T03:20:00.000Z' }),
    ], 'deploy', NOW);
  ...
  it('requires reconfirmation for equally current conflicting authenticated intents', () => {
    const resolved = resolveTemporalIntent([intent({ id: 'a', intentHash: 'ship-a' }), intent({ id: 'b', intentHash: 'ship-b' })], 'deploy', NOW);
...
describe('governed memory', () => {
  it('lets authenticated Current You correct a FutureYou preference', () => {
    const result = adjudicateMemoryWrite(
      preferenceMemory({ id: 'future-preference', source: 'future_you', factHash: 'old-preference', authenticated: false, observedAt: '2026-08-16T03:10:00.000Z' }),
      preferenceMemory({ id: 'current-correction', factHash: 'new-preference' }),
      NOW,
    );
    expect(result).toMatchObject({ decision: 'supersede', winnerId: 'current-correction', loserId: 'future-preference' });
  });
  ...
  it('does not let FutureYou overwrite authenticated Current You preference', () => {
    const result = adjudicateMemoryWrite(
      ...
      preferenceMemory({ id: 'future-write', source: 'future_you', factHash: 'projected-preference', authenticated: false, observedAt: '2026-08-17T03:20:00.000Z' }),
      NOW,
    );
    expect(result).toMatchObject({ decision: 'preserve_existing', winnerId: 'current-preference', loserId: 'future-write' });
  });
  ...
  it('does not let a user assertion overwrite verified provider runtime evidence', () => {
    const result = adjudicateMemoryWrite(
      ...
      runtimeMemory({ id: 'user-runtime-claim', source: 'current_user', factHash: 'production-healthy', observedAt: '2026-08-17T03:20:00.000Z' }),
      NOW,
    );
  ...
  it('lets a valid incoming observation supersede malformed existing memory', () => {
    const result = adjudicateMemoryWrite(runtimeMemory({ id: 'broken', observedAt: 'not-a-time' }), runtimeMemory({ id: 'fresh' }), NOW);
  ...
  it('blocks stale verified runtime memory from authorizing consequential action', () => {
    const result = memoryCanAuthorize(runtimeMemory({ observedAt: '2026-08-14T03:10:00.000Z', lastVerifiedAt: '2026-08-14T03:10:00.000Z' }), 'consequential', NOW);
...
describe('bound execution authorization', () => {
  it('accepts fresh unused authorization bound to exact intent meaning, proposal, action, scope, risk, and version', () => {
    expect(authorizationSupportsAction(authorization(), authorizationRequest()).allowed).toBe(true);
  });

  it('rejects an authorization when the same intent id has changed meaning', () => {
    const result = authorizationSupportsAction(authorization(), authorizationRequest({ intentHash: 'current-intent-v2' }));
    expect(result.allowed).toBe(false);
    expect(result.reason).toContain('identity or meaning');
  });

  it('rejects approval copied to a different action risk', () => {
    const result = authorizationSupportsAction(authorization(), authorizationRequest({ risk: 'reversible' }));
    expect(result.allowed).toBe(false);
    expect(result.reason).toContain('different action risk');
  });

  it('rejects approval replay against a different action', () => {
    const result = authorizationSupportsAction(authorization(), authorizationRequest({ actionHash: 'd'.repeat(64) }));
    expect(result.allowed).toBe(false);
    expect(result.reason).toContain('different action');
  });

  it('rejects an already-consumed authorization', () => {
    const result = authorizationSupportsAction(authorization(), authorizationRequest({ replayState: 'consumed' }));
    expect(result.allowed).toBe(false);
    expect(result.reason).toContain('already been consumed');
  });

  it('fails closed when replay state is unavailable', () => {
    const result = authorizationSupportsAction(authorization(), authorizationRequest({ replayState: 'unknown' }));
    expect(result.allowed).toBe(false);
    expect(result.reason).toContain('unused status must be proven');
  });

  it('rejects stale authorization even when hashes still match', () => {
    const result = authorizationSupportsAction(authorization({ issuedAt: '2026-08-17T01:00:00.000Z', expiresAt: '2026-08-17T01:30:00.000Z' }), authorizationRequest());
    expect(result.allowed).toBe(false);
...
describe('governed action contract', () => {
  it('allows consequential action only when intent, objective memory, proof, bound authorization, replay state, and rollback align', () => {
    ...
    expect(verdict.lineage).toMatchObject({
      intentId: 'intent-current', memoryIds: ['memory-runtime'], proofIds: ['proof-runtime'], recoveryPlanId: 'recovery-1',
      authorizationId: 'authorization-1', proposalId: 'proposal-1', proposalHash: PROPOSAL_HASH, actionHash: ACTION_HASH, exactVersion: 'abc123',
    });
  });

  it('fails closed if consequential execution does not prove authorization is unused', () => {
    const verdict = evaluateGovernedAction({
      requiredScope: 'deploy', risk: 'consequential', intents: [intent()], recoveryPlan: recovery(), proposalId: 'proposal-1', proposalHash: PROPOSAL_HASH,
      actionHash: ACTION_HASH, exactVersion: 'abc123', authorization: authorization(), now: NOW,
    });
    expect(verdict.decision).toBe('reconfirm');
    expect(verdict.reasons.join(' ')).toContain('replay state is unknown');
  });

  it('keeps FutureYou advisory instead of silently authorizing consequential action', () => {
    const verdict = evaluateGovernedAction({
      requiredScope: 'deploy', risk: 'consequential', intents: [intent({ source: 'future_you', authenticated: false })], recoveryPlan: recovery(), proposalId: 'proposal-1',
      proposalHash: PROPOSAL_HASH, actionHash: ACTION_HASH, authorization: authorization(), authorizationReplayState: 'unused', now: NOW,
    });
    expect(verdict.decision).toBe('reconfirm');
  ...
  it('rejects consequential proof with no explicit freshness boundary', () => {
    const verdict = evaluateGovernedAction({
      requiredScope: 'deploy', risk: 'consequential', intents: [intent()], proofs: [proof({ freshForMs: null, expiresAt: null })],
      requiredClaims: [{ claim: 'production_sha_matches', exactVersion: 'abc123' }], recoveryPlan: recovery(), proposalId: 'proposal-1', proposalHash: PROPOSAL_HASH,
      actionHash: ACTION_HASH, exactVersion: 'abc123', authorization: authorization(), authorizationReplayState: 'unused', now: NOW,
    });
    expect(verdict.decision).toBe('reconfirm');
  ...
  it('denies consequential action when rollback is structurally incomplete', () => {
    const verdict = evaluateGovernedAction({
      requiredScope: 'deploy', risk: 'consequential', intents: [intent()], recoveryPlan: recovery({ checkpointRef: null }), proposalId: 'proposal-1',
      proposalHash: PROPOSAL_HASH, actionHash: ACTION_HASH, authorization: authorization(), authorizationReplayState: 'unused', now: NOW,
    });
  ...
  it('denies irreversible autonomous action even when other controls are present', () => {
    const verdict = evaluateGovernedAction({
      requiredScope: 'delete-production', risk: 'irreversible', intents: [intent({ scope: ['delete-production'] })], recoveryPlan: recovery({ level: 'R4' }), proposalId: 'proposal-1',
      proposalHash: PROPOSAL_HASH, actionHash: ACTION_HASH, authorization: authorization({ scope: ['delete-production'], risk: 'irreversible' }), authorizationReplayState: 'unused', now: NOW,
    });
    expect(verdict.decision).toBe('deny');
  ...
  it('hard constraints outrank fresh authenticated Current You authorization', () => {
    const verdict = evaluateGovernedAction({
      requiredScope: 'deploy', risk: 'consequential', intents: [intent()], recoveryPlan: recovery(), proposalId: 'proposal-1', proposalHash: PROPOSAL_HASH,
      actionHash: ACTION_HASH, authorization: authorization(), authorizationReplayState: 'unused', hardConstraintViolations: ['requested action violates a non-overridable safety policy'], now: NOW,
    });
    expect(verdict.decision).toBe('deny');
