-- Postgres ON CONFLICT cannot use a deferrable unique constraint as an
-- arbiter. The outbox uses ON CONFLICT for coalescing, so make the existing
-- constraint immediate/non-deferrable.
...
-- Atomically acquire a missing or expired lease. A live lease returns false.
