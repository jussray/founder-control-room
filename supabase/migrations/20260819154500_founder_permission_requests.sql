-- Durable founder permission broker.
-- Requests carry no execution authority. Only an explicit founder decision can
-- produce the canonical FounderControlDecision receipt consumed by later gates.
...
comment on table public.founder_permission_requests is
  'Service-role-only ledger for exact-scope founder permission requests and decisions. A row does not satisfy independent review.';
