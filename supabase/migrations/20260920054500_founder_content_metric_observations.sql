...
--
-- Stores normalized observations only. Raw CSV is never durable state.
...
begin
  ...
  loop
    ...
    -- Insert first and let the unique business identity serialize concurrent imports.
    -- A concurrent exact retry becomes a read/compare path instead of surfacing a
    -- spurious unique-violation failure.
    ...
    on conflict on constraint founder_content_metric_observations_business_identity_key
    do nothing
    ...
    if found then
      ...
      if not found then
        raise exception 'FOUNDER_CONTENT_METRICS_IDEMPOTENCY_RECONCILIATION_FAILED';
      end if;
  ...
  insert into public.founder_content_post_events (
    ...
    jsonb_build_object(
      ...
      'idempotent_retry', inserted_count = 0,
