-- Scope the repair-mission policy trigger strictly to missions explicitly
-- created from repository verification. A missing source must not fall through
-- PostgreSQL's three-valued NULL logic and mutate unrelated mission types.
...
begin
  if coalesce(v_snapshot->>'source', '') <> 'repository_verification' then
    return new;
  end if;
...
comment on function public.enforce_repository_repair_mission_policy() is
  'Applies zero-budget, free-first, proposal-only boundaries only when policy_snapshot.source is repository_verification.';
