...
export const ALLOWED_REPOSITORY = 'jussray/l99-StoryEngine' as const;
...
function textArray(
  ...
  return value.map((item, index) => {
    const cleaned = text(item, `${field}_${index}`, maxLength);
    if (validator && !validator(cleaned)) throw new Error(`${field}_${index}_invalid`);
    return cleaned;
  });
}

function observedAt(value: unknown, now: number): string {
  const cleaned = text(value, 'observed_at', 64);
  const timestamp = Date.parse(cleaned);
  if (!Number.isFinite(timestamp)) throw new Error('observed_at_invalid');
  if (timestamp - now > 5 * 60 * 1000) throw new Error('observed_at_in_future');
  if (now - timestamp > 24 * 60 * 60 * 1000) throw new Error('observed_at_stale');
  return new Date(timestamp).toISOString();
}

function gateResults(value: unknown): GateResult[] {
  if (!Array.isArray(value) || value.length > 20) throw new Error('gate_results_invalid_array');
  const seen = new Set<string>();
  return value.map((item, index) => {
    const row = object(item);
    const gate = text(row.gate, `gate_results_${index}_gate`, 80);
    if (!/^[a-z0-9_-]+$/i.test(gate) || seen.has(gate)) {
      throw new Error(`gate_results_${index}_gate_invalid`);
    }
    seen.add(gate);
...
export function validateStatusEnvelope(
  ...
  const commit = text(row.commit, 'commit', 40).toLowerCase();
  ...
  const riskLevel = text(row.risk_level, 'risk_level', 16);
  const gateStatus = text(row.gate_status, 'gate_status', 16);
  ...
  const results = gateResults(row.gate_results);
  const derived = results.length === 0
    ? 'unknown'
    : results.every((result) => result.passed) ? 'pass' : 'fail';
  if (gateStatus !== derived) throw new Error('gate_status_mismatch');

  if (!Number.isInteger(row.source_run_attempt) || Number(row.source_run_attempt) < 1) {
  ...
  return {
    ...
    commit,
    observed_at: observedAt(row.observed_at, now),
    status,
    risk_level: riskLevel,
    gate_status: gateStatus as L99StatusEnvelope['gate_status'],
    gate_results: results,
    proof_refs: textArray(row.proof_refs, 'proof_refs', 20, 220, (item) => SAFE_REF_PATTERN.test(item)),
    blockers: textArray(row.blockers, 'blockers', 20, 360),
    next_gate: text(row.next_gate, 'next_gate', 700),
    source_run_id: text(row.source_run_id, 'source_run_id', 64),
    source_run_attempt: Number(row.source_run_attempt),
  };
}
...
export function evidenceStatus(envelope: L99StatusEnvelope): 'pass' | 'fail' | 'warn' | 'pending' {
  ...
  if (envelope.status === 'at-risk' || ['high', 'critical'].includes(envelope.risk_level)) return 'warn';
