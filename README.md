...
- a security-preserving GitHub ruleset membrane for existing non-FCR named rulesets that now fails closed before provider mutation: it rejects requested target refs that remain explicitly excluded, rejects explicit bypass replacement through the narrow `RulesetConfig`, and refuses the remaining existing-ruleset update until provider-supported atomic concurrency or an equivalent reconciliation contract can prevent a stale read-modify-write snapshot from overwriting newer provider hardening; brand-new rulesets and FCR's constitutional `main` path retain their existing canonical semantics, and source logic does not itself prove live provider configuration;
