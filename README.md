...
The bounded Access split keeps Cloudflare Access in the architecture without making it the public product login. The source recovery contract preserves the existing Worker/private Access application and its policies, narrows that protected application to the Worker destination, and creates a separately named public Access application whose Everyone Bypass policy is limited to `foundercontrolroom.org/*`, `www.foundercontrolroom.org/*`, and the exact `api.foundercontrolroom.org/version` release witness. Every other direct `api.foundercontrolroom.org/*` route remains behind Access. Normal browser API traffic reaches the canonical Worker privately through the Pages `FCR_API` Service Binding, so making the FCR doorway public does not make the Worker generally public. Source wiring is not live provider proof; the split still requires exact-current provider readback, separately authorized apply, and anonymous browser/runtime verification.
