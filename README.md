...
- a fail-closed GitHub compatibility membrane for non-FCR named rulesets: existing rulesets are never PUT-updated through `SecurityPreservingGitHubProvider` until a separately reviewed concurrency-safe reconciliation contract exists; requested targets still covered by exact, wildcard, or broad exclusions fail closed; explicit bypass replacement on an existing ruleset fails closed; and a missing named ruleset is created through one direct `createRepoRuleset` call whose same-name conflict propagates without update fallback. FCR's constitutional `main` path retains canonical `GitHubProvider` semantics, and source logic does not itself prove live provider configuration;
