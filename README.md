...
- a sanitized [Unified Memory Spine](docs/UNIFIED_MEMORY_SPINE.md) that normalizes bounded cross-system continuity while leaving native products as source owners; raw observations remain `untrusted-import`, cannot self-authenticate or grant decision/execution authority, metadata-only privacy boundaries fail closed, and any future consequential use must revalidate current project/runtime authority outside caller-controlled memory payloads;
- a security-preserving GitHub ruleset membrane for non-FCR named rulesets that fails closed instead of reconstructing and PUT-updating an existing provider ruleset from an earlier read until a separately reviewed concurrency-safe reconciliation contract exists; exact normalized requested-ref exclusions and `~ALL` may be diagnosed without mutation, wildcard and `~DEFAULT_BRANCH` semantics are not invented locally, missing rulesets may use the direct create-only path, non-empty bypass replacement remains fail-closed, FCR's constitutional path remains delegated to the canonical provider behavior, and source logic does not itself prove live provider configuration;
- an FCR constitutional two-membrane GitHub governance contract in source: the current founder-only human-review phase requires zero native GitHub approving reviews and no Code Owner or last-push human approval, while deterministic independent review, authenticated Founder Final, review-thread resolution, the exact CodeQL `high_or_higher` / `errors` floor, force-push/deletion protection, and pull-request-only trusted-App bypass remain load-bearing; strict required-status freshness stays isolated in a separate zero-bypass companion containing `Required Gate` plus `Verify test-ledger contract`, a future positive native-review count restores Code Owner and last-push approval, and source configuration never substitutes for live provider readback;
...
- a deterministic-review V1 producer, App-backed witness path, and founder-runtime ignition shape that keep review receipts proposal-only, derive review identity from provider-observed exact PR/base/head/diff state, block trust-root self-certification and unprovable rename provenance, reject PAT-only witness authority, require exact-head provider readback from the trusted numeric `GITHUB_APP_ID`, and use reconcile-before-create so a retry can reuse an already-existing exact trusted witness instead of blindly duplicating a Check Run. The runtime `POST /review/deterministic-witness/:pullRequestNumber` surface is usable as trusted ignition only after that code is lawfully integrated and deployed as exact current FCR `main`, the running full `GIT_SHA` equals provider-resolved `main` before and after publication/reconciliation, and the merged release identity is re-observed; candidate previews, stale releases, and the route response itself cannot supply bootstrap, founder-final, merge, deploy, secret, database, policy, billing, publication, or destructive-action authority;
...
- an FCR-owned provider-neutral n8n authority adapter that preflights transport/provider configuration before consuming one-shot authority, atomically claims the same stored exact Current You approval used by the first-party lane, and then delegates to the existing n8n cadence/idempotency/receipt path; initial reservations and pre-provider rearms consume the authoritative database-returned `approval_executions.started_at` generation, and abort, claim-boundary, provider-write, and finalization mutations remain fenced to that exact generation so a stale worker cannot fail, dispatch, or finalize a newer rearm; n8n still cannot claim final publication truth, and source presence does not prove the live n8n workflow, Buffer credentials, provider mutation, readback, or deployed exact SHA;
...
When a newly verified fingerprint supersedes a contradictory current-state assertion, delete or replace that stale assertion in the same bounded change instead of leaving two competing present-tense truths. Preserve historical commits, exact SHAs, receipts, and evidence as provenance. A bounded continuity cookie may bind predecessor fingerprint, replacement base/head/scope, proof, and next gate, but the cookie itself is never approval, provider truth, or permanent authority.
...
The current **source-requested GitHub human-review phase is founder-only**: the canonical native approving-review count is zero, and Code Owner plus last-push human approval are disabled only for that current phase. This does not remove deterministic independent-review evidence or authenticated Founder Final from the FCR merge path, and it does not remove `Required Gate`, `Verify test-ledger contract`, CodeQL, review-thread resolution, stale-review handling, trusted-App identity, force-push/deletion protection, or exact provider readback. A deliberate future positive native-review count restores Code Owner and last-push approval automatically. This source policy is not a claim that the live GitHub ruleset has already changed; live provider truth remains whatever a fresh ruleset readback proves until trusted exact-current-main reconciliation succeeds.
...
Trusted GitHub App witness/governance workflows that need GitHub App execution credentials may use the Actions-facing `APP_ID` and `APP_PRIVATE_KEY` names and map them into the stable provider/runtime names `GITHUB_APP_ID` and `GITHUB_PRIVATE_KEY` inside their bounded jobs. **Canonical production Deploy is different:** it preserves the Worker's provider-held `GITHUB_APP_ID` / `GITHUB_PRIVATE_KEY` runtime secrets and does not copy them through the GitHub deployment plane. Seeing either naming surface in source proves wiring only, not that values exist, match the intended App, can mint an installation token, are accepted by GitHub, authorize deployment, or match a deployed runtime. The GitHub App Client ID is not consumed by this installation-token path.
...
Canonical Deploy now keeps one authority per secret plane. Its GitHub `production` pre-mutation gate requires only `SUPABASE_DB_URL`, `CLOUDFLARE_API_TOKEN`, and `CLOUDFLARE_ACCOUNT_ID`, which are deployment-plane credentials. Worker runtime secret values remain provider-held in Cloudflare and are declared by `wrangler.worker.toml [secrets].required`; that required-name set includes `FOUNDER_SESSION_ENCRYPTION_KEY`. The Deploy workflow does not re-upload the service-role key, publishable key, session key, webhook secret, GitHub App pair, MCP tokens, Zapier provider hook, or review-email ingress secret. It actively writes only the fail-closed `FOUNDER_SIGNAL_AUTOMATION_GRANT_JSON` with `enabled:false`.
