...
## Deploy versus publication authority

Canonical `.github/workflows/deploy.yml` owns bounded production deployment and release verification only. Its Proof-of-Ship stage may verify the exact deployed release and retain a founder-review artifact, but Deploy keeps `PUBLISH_ALLOWED=false` and does not receive the Zapier catch-hook or Proof-of-Ship publication-grant secret. A deployment approval therefore cannot schedule or publish founder content. Publication remains a separate authority class and requires its own exact reviewed-content approval and provider/outcome proof; the separate Proof-of-Ship publication workflow remains fail-closed until that approval can be resolved from durable authority state.
