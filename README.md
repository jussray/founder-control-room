...
- a deterministic-review V1 producer and witness path that derives proposal-only review receipts from provider-observed exact PR/base/head/diff state, blocks trust-root self-certification and unprovable rename provenance, permits the exact success Check Run only through the FCR repository provider when a repository-scoped GitHub App installation credential is present, rejects PAT-only witness authority, and requires exact-head provider readback from the trusted numeric `GITHUB_APP_ID` before treating the witness as current evidence; the founder-trigger candidate adds `POST /review/deterministic-witness/:pullRequestNumber` inside the existing same-origin, authenticated-founder, `fcr-privileged-execution-master`, and repository-rate-limit membrane, accepts only a positive PR number, and keeps repository/provider/base/head/diff/verdict/hash/check/App identity server/provider-derived; a successful trigger response remains proposal-only and non-authorizing, and source integration does not prove that the exact merged release is deployed or runtime-available; this path does not itself provide bootstrap, founder-final, merge, deploy, secret, database, policy, billing, publication, or destructive-action authority;
