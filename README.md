...
- a fail-closed GitHub ruleset compatibility membrane for existing non-FCR named rulesets: source may read the current provider ruleset and rejects any requested protected ref that remains explicitly excluded, but it does not issue an update for an existing non-FCR ruleset until a separately reviewed reconciliation contract can prove concurrency safety; explicit bypass replacement also remains blocked because the narrow request shape cannot represent provider bypass mode; brand-new rulesets and FCR's constitutional `main` path retain their existing canonical semantics, and source logic does not itself prove live provider configuration;
