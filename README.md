...
- an FCR constitutional two-membrane GitHub governance contract in source: the review membrane must preserve Code Owner review plus the exact CodeQL `high_or_higher` / `errors` floor while review freshness, last-push approval, thread resolution, force-push/deletion protection, and pull-request-only trusted-App bypass are enforced; strict required-status freshness remains isolated in a separate zero-bypass companion, and source configuration never substitutes for live provider readback;
