...
- an FCR constitutional two-membrane GitHub governance contract with explicit human-review phases: current `founder_only` requires exactly zero outside approving reviews and disables stale-review dismissal, Code Owner review, and last-push approval only in the review membrane, while future `independent_review` requires one-or-more outside approvals and reenables those controls; both phases preserve pull-request enforcement, review-thread resolution, the exact CodeQL `high_or_higher` / `errors` floor, force-push/deletion protection, pull-request-only trusted-App bypass, and the separate zero-bypass strict required-status companion; zero outside approvals cannot be inferred from a missing phase, and source configuration never substitutes for live provider readback;
...
The live GitHub human-review phase is a separate provider-policy layer. `founder_only` means the repository ruleset requires no second human who does not yet exist; it does not remove FCR's deterministic review evidence, exact-head machine proof, founder-final binding, or last-moment provider revalidation. `independent_review` later adds the outside/code-owner human gate without changing that machine/evidence floor.
...
The canonical phase-aware live reconciliation command is `/reconcile-fcr-governance-phase <40-char-current-main-sha> <founder_only|independent_review> <approval-reference>` on governance issue #418. The command is founder-identity-bound, exact-current-main-bound, derives the review count from the phase enum, preserves the zero-bypass strict status membrane plus CodeQL/force-push/deletion/pull-request protections, and requires provider readback `READY` before completion. It authorizes only that bounded provider-policy reconciliation.
