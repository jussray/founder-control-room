...
- a pre-mutation production rollback receipt that must capture the currently active Worker deployment/version traffic distribution, live Worker SHA, and current successful Pages production deployment ID after exact-head authority validation and before Supabase, Worker, or Pages mutation; failure to capture provider-native rollback coordinates blocks release, while the receipt itself remains rollback-preparation evidence rather than rollback execution or rollback-success proof;
...
Before any production mutation, the canonical manual `Deploy` lane must also retain `fcr-production-rollback-receipt@v1`, containing the active Worker deployment/version distribution, live Worker SHA, and current successful Pages production deployment ID. The snapshot is read-only and runs after exact-head authority but before Supabase, Worker, or Pages mutation. Missing rollback coordinates fail closed. Receipt existence proves rollback preparation only; rollback remains a separate authorized provider action followed by fresh runtime/browser proof.
