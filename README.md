...
For Founder Control Room itself, the final provider transport uses GitHub's asynchronous pull-request merge path only after the existing provider-backed PR identity, deterministic-review, founder-final, and last-moment exact base/head freshness checks have passed. The async request is bound to the exact reviewed 40-character head SHA, an already-pending request is reconciled by provider UUID instead of blindly replayed, and success requires authoritative terminal provider state with a valid merge SHA. The FCR wrapper remains pinned to a reviewed candidate whose direct base is `main`, so this transport cannot silently authorize lower, separately unapproved PRs beneath that candidate. A queued UUID, provider acceptance, mergeability flag, source test, or successful polling setup is not merge authority or terminal merge proof.
