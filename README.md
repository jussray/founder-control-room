...
Branch creation through `src/http/routes/approvals.ts` is now an exact-action governed repository mutation. A fresh `create_branch` proof and authenticated founder execute request cause FCR to issue a server-derived `AuthorityEnvelopeV1` bound to canonical capability `github.repository.create-branch`, repository scope, exact branch arguments, current mission-state fingerprint, tool-call identity, expiry, founder identity, and idempotency key. Authority issuance inside `src/founder-os-lab` remains deterministic: it reads neither wall clock nor randomness, defaults issuance to the server-observed proof timestamp, and derives the tool-call identity from the already-bound idempotency key unless the runtime supplies a stricter value. FCR reserves the execution before the external write, re-reads mission state and runtime time immediately before mutation, and `executeAuthorizedCreateBranch()` must reject capability, scope, argument, state, tool-call, expiry, idempotency, or envelope-integrity drift before `RepositoryProvider.createBranch(...)` can be reached. A pending or ambiguous execution remains reconcile-before-retry; source and CI proof of this membrane do not by themselves prove that a live GitHub branch was created.
