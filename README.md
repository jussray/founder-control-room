...
- an existing non-FCR GitHub ruleset fail-closed membrane: the request layer preserves the semantic difference between omitted `bypassActors` and an explicit `bypassActors: []`, a non-empty bypass replacement fails closed, requested protected refs that remain explicitly excluded are rejected, and any existing-ruleset mutation, including an explicit empty bypass clear, is blocked until a separately reviewed concurrency-safe provider reconciliation contract exists; brand-new ruleset creation and FCR's constitutional `main` path retain their existing canonical semantics, and source logic does not itself prove live provider configuration;
