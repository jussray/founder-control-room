...
The required merge ledger and PR continuity machinery are part of the merge trust root, not ordinary helpers. A candidate that changes `.github/workflows/control-room-test-ledger.yml`, `.github/workflows/pr-continuity.yml`, `scripts/control-room-test-ledger.mjs`, `scripts/founder-merge-approval.mjs`, `scripts/pr-continuity.mjs`, or `scripts/verify-pr-continuity-rollover-outcome.mjs` must be classified as P1 trust-root self-modification and cannot use the normal deterministic reviewer to certify the gate it is changing. It must refresh merge-authority documentation, finish exact-head machine proof, and use the separately explicit exact-candidate constitutional/manual-override path under issue #418 before integration. Ordinary `approved`, `cont`, mergeability, green checks, or predecessor approval cannot invoke that exception.

For Chief governance, FCR contains a **read-only trusted observation and verification boundary** pinned to `jussray/chief-ai-machine` and Chief ruleset IDs `20818149` and `21261587`. It uses the repository-scoped FCR GitHub App installation-token path rather than caller-supplied PAT/token authority, preserves required-check `integration_id` producer identity, requires complete bypass and deployment readback, and fingerprints the provider observation. Under the current founder decision, ruleset `20818149` is accepted exactly as observed when it preserves zero bypass actors, the approved source checks, `Cloudflare Production`, `proofmode-access-admin`, and the unbound reserved candidate runtime context. A compliant observation returns `NO_CHANGE_REQUIRED` with `mutation:null`; drift blocks verification rather than producing a desired-state rewrite. This boundary never grants provider mutation, merge, deploy, or execution authority.
