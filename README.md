...
- an existing non-FCR GitHub ruleset fail-closed membrane: requested protected refs that remain explicitly excluded are rejected, existing ruleset mutation is blocked until a separately reviewed concurrency-safe provider reconciliation contract exists, and explicit bypass replacement remains fail-closed; brand-new ruleset creation and FCR's constitutional `main` path retain their existing canonical semantics, and source logic does not itself prove live provider configuration;
