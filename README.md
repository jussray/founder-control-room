...
A peer-only refresh is itself a truth transition. Even when the executable change is only a new exact StoryEngine evidence identity, the same reviewed range must refresh this README, `docs/TRUTH_DECAY_AUDIT.md`, and `docs/DOCUMENTATION_TRUTH_RECEIPT.json`. Until the successor exact FCR head completes the full federation witness, predecessor browser green stays historical and the successor federation state remains `UNKNOWN`.
