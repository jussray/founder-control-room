...
- an FCR-owned provider-neutral n8n authority adapter that preflights transport/provider configuration before consuming one-shot authority, atomically claims the same stored exact Current You approval used by the first-party lane, and then delegates to the existing n8n cadence/idempotency/receipt path; every prepared worker carries the authoritative `approval_executions.started_at` reservation generation so an older worker cannot abort or finalize a rearmed generation, and n8n still cannot claim final publication truth; source presence does not prove the live n8n workflow, Buffer credentials, provider mutation, readback, or deployed exact SHA;
