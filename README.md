...
- a read-only deterministic exact-head review producer that derives provider-backed PR/base/head identity, complete diff and canonical hash bindings, server-owned founder-final policy identity, versioned machine findings, verdict, and a proposal-only independent-review receipt while failing closed on moved/incomplete state and trust-root self-modification; the producer does not publish the required GitHub App Check Run witness, use provider credentials, grant founder-final or merge authority, deploy, or mutate provider state;
...
The deterministic review producer may construct the exact proposal-only receipt from provider-grounded repository/PR/base/head/diff identity and server-owned policy/rule inputs, but that receipt is not the witness. The producer fails closed on moved/incomplete state and on self-modification of its own trust root. A separate publisher must create the exact receipt-derived Check Run through the repository-scoped server-owned GitHub App, then provider readback must prove the expected head/name/status and App issuer identity before the deterministic review gate can pass.
