...
- for an existing non-FCR named GitHub ruleset, repository-owned mutation is currently fail-closed: the compatibility provider may read current state to classify the request, but it rejects requested protected refs that remain explicitly excluded, rejects explicit bypass replacement through the narrow `RulesetConfig`, and does not issue an existing-ruleset provider `PUT` until a separately reviewed reconciliation contract can prove concurrency safety; brand-new ruleset creation and FCR's constitutional `main` path retain their existing canonical semantics, and source logic does not itself prove live provider configuration;
...
- a repository-owned Worker build authority membrane that binds native Workers Builds to the checked-out Git source SHA, permits only non-promoting native version uploads, and reserves production promotion for exact manual GitHub release authority without claiming live provider configuration;
