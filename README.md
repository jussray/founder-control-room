...
For Chief governance, FCR contains a **read-only trusted observation and reconciliation-planning boundary** pinned to `jussray/chief-ai-machine` and Chief ruleset IDs `20818149` and `21261587`. It uses the repository-scoped FCR GitHub App installation-token path rather than caller-supplied PAT/token authority, preserves required-check `integration_id` identity, requires complete bypass and deployment readback, and fingerprints the complete provider observation. The September 13 founder decision supersedes the former as-is topology: source now plans `Verify candidate ProofMode runtime with Playwright` on no-bypass ruleset `20818149` bound to GitHub Actions integration `15368`, narrows the governance-boundary unconditional merge checks to `Typecheck` plus `Verify operational authority`, requires zero desired bypass actors and strict status freshness, and removes `Cloudflare Production` plus `proofmode-access-admin` from the pre-merge desired state while retaining both as explicit post-merge production-proof obligations. Integration `15368` authenticates the GitHub Actions App identity but does **not** by itself prove the exact workflow file or event that produced a check; workflow provenance remains separate evidence. The plan is fingerprint-bound and non-authorizing. Existing non-FCR ruleset mutation remains fail-closed until a separately authorized drift-safe provider execution design preserves unmanaged security and receives fresh post-write readback. This boundary never grants provider mutation, merge, deploy, or execution authority.
