...
- a security-preserving GitHub ruleset update membrane for existing non-FCR named rulesets that may narrow requested required-status contexts without silently dropping stronger provider-owned review, code-scanning, history, Copilot, deletion, force-push, bypass, or retained check-integration protections; new rulesets and FCR's constitutional `main` path retain their existing canonical semantics, and source logic does not itself prove live provider configuration;
