...
Trusted deterministic-review dispatch resolves base freshness from live GitHub state rather than treating the pull request's `.base.sha` snapshot as current authority. The snapshot is retained only as provenance. Before dispatch, FCR re-reads live `main`, requires the pull request to remain open and target `main`, compares live `main` to the exact pull-request head, requires `behind_by=0`, and requires the merge base to equal live `main`. A historical base snapshot therefore cannot falsely block a current candidate, while a genuinely behind candidate cannot receive a trusted witness.
