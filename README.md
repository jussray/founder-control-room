...
- a provider-native founder-content readback witness that keeps an n8n `scheduled` acknowledgement non-final until a sanitized independent provider observation matches the exact orchestration, provider item/request identity, source repository/SHA, platform/channel, and requested schedule; the witness never turns scheduled state into publication truth and retains neither post copy nor provider credentials;
...
A platform-native draft is not proof that the provider adapter is live. n8n acceptance is not publication truth. A scheduled n8n receipt becomes provider-outcome evidence only after the provider-native readback witness independently matches the exact orchestration/provider item/request identity, source repository/SHA, platform/channel, and requested schedule; even then, scheduled state remains `published:false`. Provider readback remains terminal external-state evidence.
