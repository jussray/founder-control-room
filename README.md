...
- a read-only Founder Control Room GitHub PR audit MCP that reads PR/base/head/check/review/diff evidence through the current `RepositoryProvider` boundary, binds sanitized candidate and evidence fingerprints to a five-minute non-authorizing `external-read` continuity cookie, explicitly leaves provider collection completeness `not_proven`, and cannot approve, merge, comment, dispatch, deploy, publish, or mutate GitHub;
