...
- `.github/workflows/quality-gate.yml` must keep `python3 scripts/continuity_model_guard.py` and `node scripts/audit_pr_continuity_rulesets.mjs` load-bearing. The continuity guard protects source-model semantics; the ruleset audit re-reads live GitHub governance and must fail closed when all-branch release or linear-history rules block lawful PR rollover. A provider-state failure is not permission to weaken the audit, and neither check grants ruleset mutation, merge, deploy, or founder authority.
