...
The Supabase `migration-lint` job is load-bearing merge evidence. On pull requests it must checkout and verify the exact `github.event.pull_request.head.sha`, run `supabase db lint --local` as a hard failure rather than `continue-on-error`, and feed its result into `Required Gate`. Skipped, stale-head, soft-failed, or advisory database lint is not sufficient merge proof.
