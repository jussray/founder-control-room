...
function parseIsoTimestamp(value, field) {
  if (typeof value !== 'string' || !ISO_UTC.test(value) || Number.isNaN(Date.parse(value))) {
    fail(`${field} must be an ISO UTC timestamp`);
  }
  return value;
}
...
function assertNoDuplicateSnapshotProvenance(groups) {
  const seen = new Map();
  for (const group of groups.values()) {
    ...
    if (previous) {
      fail(`snapshots ${previous} and ${group.id} duplicate the same capture provenance`);
    }
    seen.set(fingerprint, group.id);
  }
}
...
function parseFounderContentAnalyticsCsv(csvText, metadata = {}) {
  ...
  assertNoDuplicateSnapshotProvenance(groups);
