...
const MAX_AUTHORIZATION_RECEIPT_LENGTH = 240;
...
const PUBLIC_SIGNAL_CONTEXT_VERSION = 'fcr/public-signal-context@v1';
const PUBLIC_SIGNAL_POLICY_VERSION = 'public-progress-v1';
const FOUNDER_APPROVED_STEERING_GRANT_ID = 'founder-approved-auto-distribution-v1';
...
const ALLOWED_CONTENT_FIELDS = new Set([
  ...
  'threads_draft', 'x_draft', 'tiktok_caption', 'youtube_shorts_draft', 'pinterest_draft',
  'bluesky_draft', 'mastodon_draft', 'google_business_draft',
]);
const FORBIDDEN_CONTENT_FIELDS = new Set([
  'prompt', 'system_prompt', 'user_prompt', 'user_message', 'instructions', 'raw_response', 'input', 'github_evidence',
  'private_evidence', 'evidence_refs', 'source_context',
]);
...
function computePublicSignalHash(input = {}) {
  const context = {
    version: PUBLIC_SIGNAL_CONTEXT_VERSION,
    post_text: asTrimmedString(input.post_text),
    channel: asTrimmedString(input.channel),
    source_commit_sha: asTrimmedString(input.source_commit_sha).toLowerCase(),
    proof_url: asTrimmedString(input.proof_url) || null,
    current_intent_hash: asTrimmedString(input.current_intent_hash).toLowerCase(),
    source_context_hash: asTrimmedString(input.source_context_hash).toLowerCase(),
    evidence_hash: asTrimmedString(input.evidence_hash).toLowerCase(),
    evidence_count: asInteger(input.evidence_count),
    policy_version: asTrimmedString(input.policy_version),
  };
...
function validateLinkedInRisingFloor(input, errors) {
  ...
  if (!linkedinRisingFloorReady) errors.push('linkedin_rising_floor_ready must be true before LinkedIn can be scheduled');
  if (!linkedinBaselineRef || LINKEDIN_UNKNOWN_BASELINE_VALUES.has(linkedinBaselineRef.toLowerCase())) {
    errors.push('linkedin_baseline_ref must name the latest verified LinkedIn analytics or platform-recap baseline');
  }
  for (const [name, value] of [
    ['linkedin_growth_hypothesis', linkedinGrowthHypothesis], ['linkedin_24h_gate', linkedin24hGate],
    ['linkedin_48h_gate', linkedin48hGate], ['linkedin_next_mutation', linkedinNextMutation],
  ]) {
    if (value.length < LINKEDIN_MIN_STRATEGY_TEXT) errors.push(`${name} must contain at least ${LINKEDIN_MIN_STRATEGY_TEXT} characters of finished strategy`);
  }
  return {
...
function validateBufferPublishInput(input = {}, options = {}) {
  ...
  const currentIntentHash = asTrimmedString(input.current_intent_hash).toLowerCase();
  const sourceContextHash = asTrimmedString(input.source_context_hash).toLowerCase();
  const evidenceHash = asTrimmedString(input.evidence_hash).toLowerCase();
  const evidenceCount = asInteger(input.evidence_count);
  const policyVersion = asTrimmedString(input.policy_version);
  const claimedPublicSignalHash = asTrimmedString(input.public_signal_hash).toLowerCase();
  ...
  if (!ALLOWED_CONTENT_FIELDS.has(contentField)) {
    errors.push(FORBIDDEN_CONTENT_FIELDS.has(contentField)
      ...
      : `content_field ${contentField || '<empty>'} is not an approved social output field`);
  }
  if (!channel) errors.push('channel is required');
  let linkedinStrategy = null;
  if (channel === LINKEDIN_CHANNEL) {
    if (contentField !== 'linkedin_draft') errors.push('juss_rayy_linkedin must publish only linkedin_draft');
    linkedinStrategy = validateLinkedInRisingFloor(input, errors);
  }
  if (!ALLOWED_DESTINATION_MODES.has(destinationMode)) errors.push('destination_mode must be schedule under the 20-minute review-window contract');
  if (!publishAllowed) errors.push('publish_allowed must be true for the approved schedule-with-review-window contract');
  ...
  } else if (steeringGrantId !== FOUNDER_APPROVED_STEERING_GRANT_ID) {
    errors.push('steering_grant_id must match the checked-in founder standing policy');
  }
  if (founderApprovalId.length > MAX_AUTHORIZATION_RECEIPT_LENGTH) errors.push(`founder_approval_id must not exceed ${MAX_AUTHORIZATION_RECEIPT_LENGTH} characters`);
  if (authorizationMode !== BUFFER_AUTHORIZATION_MODE) errors.push('authorization_mode must be standing-policy');
  if (schedulePolicyId !== BUFFER_SCHEDULE_POLICY_ID) errors.push('schedule_policy_id does not match the checked-in scheduling contract');
  if (notificationMode !== BUFFER_NOTIFICATION_MODE) errors.push('notification_mode must be gmail_campaign_digest');
  if (!UUID.test(batchId)) errors.push('batch_id must be a UUID');
  if (batchSize === null || batchSize < 1 || batchSize > 3) errors.push('batch_size must be an integer from 1 through 3');
  if (batchIndex === null || batchIndex < 1 || batchSize === null || batchIndex > batchSize) errors.push('batch_index must identify one item inside the declared batch_size');
  if (postText.length < MIN_POST_LENGTH) errors.push(`post_text must contain at least ${MIN_POST_LENGTH} characters of finished copy`);
  if (PROMPT_LEAK_PATTERNS.some((pattern) => pattern.test(postText))) errors.push('post_text resembles instructions, a prompt template, or unresolved automation input');
  ...
  if (!SHA256.test(currentIntentHash)) errors.push('current_intent_hash must be a SHA-256 hash');
  if (!SHA256.test(sourceContextHash)) errors.push('source_context_hash must be a SHA-256 hash');
  ...
  if (policyVersion !== PUBLIC_SIGNAL_POLICY_VERSION) errors.push('policy_version must match the checked-in public signal policy');
  if (!SHA256.test(claimedPublicSignalHash)) errors.push('public_signal_hash must be a SHA-256 hash');

  const computedPublicSignalHash = computePublicSignalHash({
    post_text: postText, channel, source_commit_sha: sourceCommitSha, proof_url: proofUrl,
    current_intent_hash: currentIntentHash, source_context_hash: sourceContextHash,
    evidence_hash: evidenceHash, evidence_count: evidenceCount, policy_version: policyVersion,
  });
  if (SHA256.test(claimedPublicSignalHash) && claimedPublicSignalHash !== computedPublicSignalHash) {
    errors.push('public_signal_hash does not match the exact public, evidence, and Current-You context');
  }
  const expectedApprovalId = steeringGrantId && invocationId && SHA256.test(claimedPublicSignalHash)
    ? `standing-policy:${steeringGrantId}:${invocationId}:${claimedPublicSignalHash.slice(0, 16)}`
    : null;
  if (!expectedApprovalId || expectedApprovalId.length > MAX_AUTHORIZATION_RECEIPT_LENGTH || founderApprovalId !== expectedApprovalId) {
    errors.push('founder_approval_id must be the context-bound standing-policy correlation receipt');
  }

  if (generatedAtMs === null) errors.push('generated_at must be a valid ISO timestamp');
  else {
    const generationAgeMs = nowMs - generatedAtMs;
    if (generationAgeMs > MAX_GENERATION_AGE_MS) errors.push('generated_at is too stale to preserve a meaningful review window');
    if (generationAgeMs < -MAX_CLOCK_SKEW_MS) errors.push('generated_at is too far in the future');
  }
  if (errors.length > 0) {
  ...
  return {
    ...
    public_signal_hash: computedPublicSignalHash,
...
if (typeof inputData !== 'undefined') output = validateBufferPublishInput(inputData);
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    validateBufferPublishInput, validateLinkedInRisingFloor, computePublicSignalHash,
    BUFFER_PROVIDER_ACTION, BUFFER_PROVIDER_METHOD, BUFFER_API_SHARING_MODE, BUFFER_API_SAVE_TO_DRAFT,
    BUFFER_REVIEW_WINDOW_MINUTES, BUFFER_SCHEDULE_POLICY_ID, BUFFER_AUTHORIZATION_MODE, BUFFER_NOTIFICATION_MODE,
    PUBLIC_SIGNAL_CONTEXT_VERSION, PUBLIC_SIGNAL_POLICY_VERSION, FOUNDER_APPROVED_STEERING_GRANT_ID,
    LINKEDIN_CHANNEL, LINKEDIN_MIN_STRATEGY_TEXT, MAX_STEERING_GRANT_ID_LENGTH, MAX_AUTHORIZATION_RECEIPT_LENGTH,
    ALLOWED_CONTENT_FIELDS, FORBIDDEN_CONTENT_FIELDS, PROMPT_LEAK_PATTERNS,
  };
}
