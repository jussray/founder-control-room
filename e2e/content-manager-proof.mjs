...
try {
  ...
  assert.equal(await founderLane.getAttribute('data-provider-write-state'), 'buffer-only');
  assert.equal(await founderLane.getAttribute('data-buffer-transport'), 'active');
  assert.equal(await founderLane.getAttribute('data-direct-linkedin-state'), 'inactive');
  ...
  assert.match(founderLaneText, /Buffer is the only active founder-content transport/i);
  assert.match(founderLaneText, /Buffer preflight happens before the one-shot approval is consumed/i);
  assert.match(founderLaneText, /Cambiante and direct LinkedIn are not active fallback paths/i);
  assert.match(founderLaneText, /publication state remains UNKNOWN/i);
  ...
  assert.equal(await founderLane.locator('button, .action').count(), 0, 'transport capability must not be presented as an already-authorized publish control');
  ...
  assert.match(learningText, /claim stays BLOCKED for first-party publication/i);
  ...
  assert.equal(await status.locator('[data-buffer-transport]').getAttribute('data-buffer-transport'), 'active');
  assert.equal(await status.locator('[data-direct-linkedin-state]').getAttribute('data-direct-linkedin-state'), 'inactive');
  ...
  assert.match(statusText, /Buffer transport active/i);
  assert.match(statusText, /Direct LinkedIn inactive/i);
  ...
  assert.match(statusText, /Buffer state UNKNOWN/i);
  ...
  assert.match(publishText, /Buffer receives only the exact FCR-approved payload/i);
  assert.match(publishText, /accepted schedule is not treated as a published LinkedIn post/i);
  assert.match(publishText, /buffer schedule receipt/i);
  ...
  assert.match(policyText, /Buffer configuration gap/i);
  ...
  assert.match(policyText, /Publication requires terminal readback/i);

  const pageText = await page.locator('main').innerText();
  assert.doesNotMatch(pageText, /First-party LinkedIn publish capability implemented/i);
  assert.doesNotMatch(pageText, /Cambiante, Buffer, or another approved actuator owns/i);
  assert.match(pageText, /Chief proposes\. FCR authorizes\. Buffer transports\. Publication truth still requires readback\./i);
