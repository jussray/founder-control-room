(() => {
  ...
  function readCapabilityPlan(actionType) {
    ...
    if (!raw) throw new Error('Attach the Chief AI V10 capability plan before privileged execution.');

    let plan;
    try { plan = JSON.parse(raw); } catch { throw new Error('Chief AI V10 capability plan must be valid JSON.'); }
  ...
  function addPlanField(form, actionType) {
    ...
    wrapper.innerHTML = `
      ...
      <textarea class="code" name="capabilityPlan" rows="8" required autocomplete="off" spellcheck="false"
        aria-describedby="v10-plan-help-${actionType}" placeholder='{"contract":"juss-v10/capability-plan@v1", ...}'></textarea>
  ...
  function completionClaim(actionType, payload) {
    ...
    if (!receipt) return { text: `${label} accepted, but completion is not claimed: execution receipt unavailable.`, status: 'unverified', evidenceKinds: [], warningCount: 0 };
    if (warnings.length > 0) return { text: `${label} executed; completion is not claimed. Evidence: execution ${receipt}. Warning: ${warnings.join(' | ')}`, status: 'incomplete', evidenceKinds: ['execution'], warningCount: warnings.length };

    if (actionType === 'merge') {
      const checks = result.evidence && typeof result.evidence === 'object' ? Object.entries(result.evidence) : [];
      ...
      return {
        text: `Merge witnessed. Evidence: execution ${receipt}; merge ${shortSha(result.mergeCommitSha)}; exact head ${shortSha(result.expectedHeadSha)}; checks ${checks.map(([kind, value]) => `${kind}=${value}`).join(', ')}.`,
        status: 'witnessed',
        evidenceKinds: ['execution', 'merge_commit', 'exact_head', 'checks'],
        warningCount: 0,
      };
    }
  ...
  function applyEvidenceBackedCompletionClaim() {
    ...
    const notice = [...document.querySelectorAll('#root .notice')].find((node) => node.textContent?.trim() === expectedText);
    if (!notice) return;
  ...
  window.fetch = async function v10BoundFetch(input, init = {}) {
    ...
    if (method !== 'POST' || !executePath.test(url.pathname) || typeof init.body !== 'string') return nativeFetch(input, init);

    let body;
    try { body = JSON.parse(init.body); } catch { return nativeFetch(input, init); }
    if (!body || !privilegedActions.has(body.actionType)) return nativeFetch(input, init);

    const nextInit = body.capabilityPlan !== undefined ? init : { ...init, body: JSON.stringify({ ...body, capabilityPlan: readCapabilityPlan(body.actionType) }) };
    const response = await nativeFetch(input, nextInit);
    try {
      const payload = await response.clone().json();
      lastExecutionEvidence = response.ok && payload?.ok === true ? { actionType: body.actionType, payload } : null;
    } catch { lastExecutionEvidence = null; }
    return response;
  };
})();