...
const state = { query: '', category: 'all', selectedId: capabilities[0].id, copied: false };
...
function sessionExists() {
  try { return Boolean(JSON.parse(sessionStorage.getItem(SESSION_KEY) ?? 'null')?.access_token); } catch { return false; }
}
...
function render() {
  if (!sessionExists()) return signInBoundary();
  const results = filtered();
  if (!results.some(item => item.id === state.selectedId)) state.selectedId = results[0]?.id ?? null;
  const selected = capabilities.find(item => item.id === state.selectedId);
