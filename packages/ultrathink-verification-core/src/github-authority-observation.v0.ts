...
function normalizeBypassActors(
  actors: readonly GitHubAuthorityBypassActorV0[],
): string[] {
  return normalize(actors.map(({ actorType, actorId, bypassMode }) =>
...
export function normalizeGitHubAuthorityObservationV0(
  ...
  return {
    ...
    requiredEvidence: readback.requiredStatusChecks === null
      ? null
      : normalize(readback.requiredStatusChecks),
    bypassPrincipals: readback.bypassActors === null
      ? null
      : normalizeBypassActors(readback.bypassActors),
  };
}
