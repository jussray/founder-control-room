...
The September 13 founder decision supersedes the earlier **as-is** Chief ruleset policy. FCR must now plan a producer-correct pre-merge topology from fresh trusted provider observations without treating source intent as live provider truth: `Verify candidate ProofMode runtime with Playwright` belongs on no-bypass ruleset `20818149` and is bound to GitHub Actions integration `15368`; the governance-boundary unconditional check set narrows to `Typecheck` plus `Verify operational authority`; desired bypass actors are zero; strict status freshness is required; and `Cloudflare Production` plus `proofmode-access-admin` leave the pre-merge desired deployment set while remaining explicit post-merge/current-main production-proof obligations. The integration binding is necessary but workflow/event provenance remains a separate evidence requirement. The reconciliation plan is bound to complete provider fingerprints, must preserve unmanaged security rules, and is non-authorizing. Existing non-FCR ruleset writes remain fail-closed until a separately reviewed drift-safe provider execution design is explicitly authorized and fresh post-write readback proves the intended provider state. Observation, a same-named check, deployment acceptance, producer capability, or a source plan never grants GitHub ruleset mutation, merge, deploy, provider-policy, production-proof, or self-certification authority.
