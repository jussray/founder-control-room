...
The external MCP `github_audit_pr` source path is a narrower GitHub evidence consumer. It is fixed to `jussray/founder-control-room`, accepts a positive PR number and optional full expected-head SHA, and uses the repository-scoped GitHub App reader to collect bounded PR, live-base, exact-head check/status, review, and changed-file evidence. It must preserve truncation/incompleteness, re-observe load-bearing PR/base/head state before calling evidence current, and fail closed when the provider observation moves or cannot prove completeness. Patch bodies, credentials, and arbitrary caller-selected repositories are outside the returned evidence model. The tool is observation only and cannot approve, comment, dispatch, merge, deploy, publish, change rulesets, or otherwise mutate GitHub. Source implementation and tests do not prove that the live App permissions, installation scope, OAuth client flow, or deployed MCP route are active.
