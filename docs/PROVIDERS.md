...
For Chief governance specifically, `src/providers/trustedGithubGovernanceObserver.ts` and `src/providers/githubGovernanceReconciliation.ts` form a narrower read-only observation and reconciliation-planning boundary. Observation is pinned to `jussray/chief-ai-machine` and ruleset IDs `20818149` and `21261587`, uses FCR's repository-scoped GitHub App installation-token path rather than accepting a caller PAT/token, preserves required-check `integration_id` producer identity, requires complete bypass-actor and required-deployment readback, accepts provider-native bypass actors whose `actor_id` is null, and fingerprints the complete provider observation. The desired topology places `Verify candidate ProofMode runtime with Playwright` on no-bypass exact-head ruleset `20818149`, binds that required context to GitHub Actions integration `15368`, and treats workflow/event provenance as separate evidence rather than inferring it from App identity. The governance-boundary unconditional checks narrow to `Typecheck` plus `Verify operational authority`; desired bypass actors are empty on both rulesets; strict status freshness is required; and `Cloudflare Production` plus `proofmode-access-admin` are removed from pre-merge desired deployments while remaining explicit post-merge/current-main production-proof obligations. The plan is bound to the trusted provider fingerprints, declares unmanaged security-rule preservation requirements, and always remains non-authorizing. Existing non-FCR ruleset mutation stays fail-closed until a separately reviewed drift-safe execution design is explicitly authorized, applies only to the observed subject, and performs fresh post-write provider readback. Neither observation nor planning grants ruleset mutation, Chief merge, FCR merge, deployment, production-proof, or execution authority, and source proof is not live Chief provider proof.
