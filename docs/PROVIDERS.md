...
Canonical founder-final integration must use the provider-backed reviewed PR number plus the exact reviewed head SHA through GitHub's asynchronous pull-request merge endpoint. `src/providers/providerFactory.ts` re-resolves the reviewed `main` base and candidate head immediately before mutation and may invoke `src/providers/githubAsyncMerge.ts` only for Founder Control Room repository identity on the reviewed direct `main` base; other repositories remain on their existing provider-neutral integration path. The async request sends the exact reviewed 40-character head SHA, reconciles an existing provider request by UUID instead of blindly enqueueing another mutation, polls to a terminal provider result, returns only a valid terminal merge SHA, and fails closed on provider failure, malformed state, or unresolved pending state. Because the FCR wrapper requires the candidate's direct base to be `main`, the selected FCR PR is the bottom of any GitHub stack and the transport cannot silently merge lower, separately unapproved PRs beneath it. Higher stack entries remain outside this exact merge action.

Async transport is not merge authority. The GitHub App is not granted an `always` bypass for direct protected-branch writes, and an async UUID, queued state, provider acceptance, or mergeability bit cannot substitute for deterministic independent review or authenticated founder-final approval bound to the exact repository, PR, base SHA, and head SHA. If either ref moves, prior proof and approval expire. If the base moves after the last local read, the no-bypass strict-freshness ruleset is the provider-side invariant that must invalidate mergeability until the candidate is current and its checks rerun. Source validation and unit mocks prove only the checked implementation; a live merge claim still requires an authorized provider mutation plus authoritative terminal readback.
