...
Canonical founder-final integration must use the provider-backed reviewed PR number plus the exact reviewed head SHA through GitHub's asynchronous pull-request merge endpoint. `src/providers/providerFactory.ts` must re-resolve the reviewed `main` base and candidate head immediately before mutation and may invoke `src/providers/githubAsyncMerge.ts` only for Founder Control Room repository identity on the reviewed direct `main` base; other repositories remain on their existing provider-neutral integration path. The async request must send the exact reviewed head SHA, reconcile an existing provider request by UUID, poll to a terminal provider result, return only a valid terminal merge SHA, and fail closed on provider failure, malformed state, or unresolved pending state. Because the FCR wrapper requires the candidate's direct base to be `main`, the selected FCR PR is the bottom of any GitHub stack and the async transport cannot silently merge lower, separately unapproved PRs beneath it. Higher stack entries remain outside this exact merge action.
