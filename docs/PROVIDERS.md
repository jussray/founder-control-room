...
In the GitHub Actions `production` environment, the credential source names are `APP_ID` and `APP_PRIVATE_KEY`. Trusted workflows map those legal Actions secret names to the stable runtime variables `GITHUB_APP_ID` and `GITHUB_PRIVATE_KEY` before provider construction. The GitHub App Client ID is not used by this installation-token path. Do not confuse a configured Client ID, a secret label, or the presence of a secret with provider proof; exact App-issuer and installation-token readback remain required.

`src/providers/githubAppAuth.ts` may normalize only transport-equivalent forms of the same complete GitHub App private key: raw PEM, escaped-newline or CRLF PEM, an optional UTF-8 BOM before the same PEM, JSON-quoted PEM, a matching single-quoted transport wrapper, and base64/base64url-encoded PEM. After normalization it must parse the value with Node `createPrivateKey`, require a private RSA key for RS256, and fail closed with a bounded non-secret configuration error when the material is malformed, incomplete, non-private, or non-RSA. Normalization is compatibility plumbing only; it does not prove that `APP_PRIVATE_KEY` belongs to the intended App, that the App installation scope is correct, that a token was minted, or that provider mutation is authorized.

### PromptOS workflow-registry writer

`src/providers/PromptOSRegistryWriter.ts` is a deliberately narrow consequential GitHub write surface for one registered action only: promoting an already-approved PromptOS workflow into `jussray/promptos` `main` by changing `workflows/registry.json`. It is not a general replacement for `RepositoryProvider.commitPatch()` and it does not create ambient repository-write authority.

The writer must receive the exact founder-approved PromptOS pre-head from the server-side execution membrane. Before preparing the write it reads live `main` and fails if the branch no longer equals that approved SHA. The new Git commit must use that exact approved SHA as its single parent, and the ref update must use `force:false`. This makes concurrent movement a provider-enforced non-fast-forward failure instead of silently rebasing the approved decision onto newer state.

Production credentials use the same repository-scoped GitHub App installation-token mechanism as other App-aware provider access; a bounded `GITHUB_TOKEN` fallback exists only for local/development compatibility. The writer is pinned to `jussray/promptos`, `main`, and `workflows/registry.json`; callers cannot supply a broader repository, branch, or path.

A provider error after the ref-update request is treated as ambiguous, never automatically retried. The writer reconciles by rereading `main`: only if provider state already equals the exact newly created commit may the result be classified `RECONCILED_AFTER_AMBIGUOUS_RESPONSE`; otherwise it fails as not applied. A final branch readback is mandatory. The surrounding founder-permission executor then independently reads the resulting registry content and exact branch head before it may emit `outcomeVerified:true`.

The source implementation proves that this bounded provider sink exists. It does **not** prove a live PromptOS promotion has occurred. A live claim requires an actual exact approved request, one-shot consumption, the provider mutation, and authoritative post-write readback. A consume/effect receipt remains evidence and never becomes reusable execution authority.

Deterministic independent-review witness publication is stricter than ordinary repository access. `src/providers/providerFactory.ts` may expose `publishDeterministicReviewWitness(...)` for Founder Control Room only when runtime `GITHUB_APP_ID` plus `GITHUB_PRIVATE_KEY` minted the repository-scoped installation token; a PAT-only or local `GITHUB_TOKEN` construction must fail closed for this capability. `src/providers/DeterministicReviewGitHubProvider.ts` may create only the exact success Check Run derived from the full review hash and exact reviewed head, and its normal runtime witness writer must remain pinned to `https://api.github.com`; a custom `GITHUB_API_BASE_URL` may exercise witness publication only through an explicitly injected test transport. `src/review/deterministicReviewWitnessPublisher.ts` must accept success only after provider readback reports the same exact-head check from the server-owned App id. The concrete witness writer, GitHub App credential minting, security-preserving provider inheritance, provider readback, provider factory/interface, deterministic producer/publisher, independent-review gate, and merge consumer form one trust chain; changes to that chain must not self-certify through the same deterministic producer. Receipt creation and Check Run publication remain proposal/evidence operations and never mint founder-final, merge, deployment, secret, provider-policy, database, billing, publication, or destructive-action authority.
...
For Chief governance specifically, `src/providers/trustedGithubGovernanceObserver.ts` and `src/providers/githubGovernanceReconciliation.ts` form a narrower evidence-only boundary. Observation is pinned to `jussray/chief-ai-machine` and ruleset IDs `20818149` and `21261587`, uses FCR's repository-scoped GitHub App installation-token path rather than accepting a caller PAT/token, preserves required-check `integration_id` producer identity, requires complete bypass-actor readback, and fingerprints the complete provider observation. Reconciliation preserves unrelated required checks and their producer bindings and may plan removal/replacement of the explicitly modeled legacy ProofMode contexts only. Because GitHub's existing-ruleset update endpoint does not provide a documented atomic conditional-update precondition for the observed state, a required mutation must fail closed as `BLOCKED_PROVIDER_ATOMIC_PRECONDITION_UNAVAILABLE`; an already-correct observation may return `NO_CHANGE_REQUIRED`. Neither observation nor reconciliation planning grants ruleset mutation, Chief merge, FCR merge, deployment, or execution authority, and source proof is not live Chief provider proof.

For active governance protecting `jussray/founder-control-room` `main`, repository identity is constitutional and mutable project slugs are not. One founder policy request is translated into **two aggregated GitHub rulesets**. In the current `founder-only` human-review phase, the review membrane owns pull-request enforcement with exactly zero native approving reviews, no Code Owner or last-push human-review requirement, stale-review dismissal semantics, review-thread resolution, the exact CodeQL code-scanning floor (`security_alerts_threshold: high_or_higher`, `alerts_threshold: errors`), force-push protection, and deletion protection. It contains exactly one App actor whose numeric ID equals trusted runtime `GITHUB_APP_ID`, and that bypass is constrained to GitHub `pull_request` mode. The review membrane contains no required-status rule. The strict-freshness companion owns exactly `Required Gate` and `Verify test-ledger contract` with `strict_required_status_checks_policy: true`, targets the same protected refs, contains zero bypass actors, and contains no pull-request, code-scanning, or other rule types. The provider applies and reads back the no-bypass freshness membrane before changing the review membrane, so a partial active reconciliation cannot weaken the old membrane.

The founder-only native-review phase does not remove deterministic independent review or authenticated Founder Final from FCR's in-app authority path. Those remain separate load-bearing evidence and human-authority layers. Native reviewer availability is observed for a future-team transition but is not a current `READY` prerequisite while the canonical native approving-review count is zero. When a deliberate future policy raises that canonical count above zero, the provider must automatically restore Code Owner review and last-push approval while preserving every existing pull-request, CodeQL, strict-freshness, trusted-App, force-push/deletion, deterministic-review, founder-final, and readback boundary.

Review readback must fail closed if the native approving-review count or Code Owner/last-push flags do not match the requested phase, if the exact CodeQL floor is absent/duplicated/weakened, if required security rule types are missing, if unmodeled rule types appear, or if the trusted bypass posture differs. A successful provider result receipts **both** ruleset component identities. If the later review write fails before GitHub returns a provider identity, the error retains the already-verified freshness ruleset name and ID. If the review write succeeds but its hardened readback then fails, the error retains **both** mutated ruleset identities for reconciliation. Missing, widened, additional, stale, non-strict, or mismatched provider readback fails closed.
...
This preserves separate membranes instead of weakening either one: the current founder-only phase removes only the nonexistent native-human reviewer dependency, while deterministic independent review, authenticated Founder Final, exact status freshness, CodeQL, provider readback, and protected-branch controls remain load-bearing. Source configuration does **not** prove that GitHub is already enforcing this phase. Until trusted exact-current-main reconciliation runs with an auditable approval reference and post-write provider readback returns `READY`, the live ruleset must be reported exactly as observed rather than relabeled to match source intent.
