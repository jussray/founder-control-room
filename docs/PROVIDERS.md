...
For active governance protecting `jussray/founder-control-room` `main`, repository identity is constitutional and mutable project slugs are not. `src/providers/fcrGovernancePhase.ts`, `src/providers/FounderControlRoomGovernanceGitHubProvider.ts`, and `src/providers/providerFactory.ts` define two explicit human-review phases for the canonical FCR ruleset while preserving one common machine/security floor.

`founder_only` is the current solo-founder phase. It requires an explicit `governancePhase: "founder_only"`, requires exactly zero outside approving reviews, and sets stale-review dismissal, Code Owner review, and last-push approval to false in the GitHub review membrane. A zero review count without the explicit phase is rejected before provider mutation, so historical or malformed configuration cannot silently weaken review policy.

`independent_review` is the later team phase. It requires at least one approving review and turns stale-review dismissal, Code Owner review, and last-push approval back on. Historical FCR configurations that already require one or more approvals remain compatible as `independent_review`; that compatibility cannot infer `founder_only`.

Both phases retain pull-request enforcement, review-thread resolution, the exact CodeQL code-scanning floor (`security_alerts_threshold: high_or_higher`, `alerts_threshold: errors`), force-push protection, deletion protection, and exactly one trusted App bypass constrained to GitHub `pull_request` mode. Neither phase moves required status checks into that bypassable review membrane. The strict-freshness companion owns exactly `Required Gate` and `Verify test-ledger contract` with `strict_required_status_checks_policy: true`, targets the same protected refs, and contains zero bypass actors. The provider applies and reads back this no-bypass freshness membrane before changing the review membrane, and phase-aware provider readback must match the requested phase exactly before reconciliation can be called complete.

The trusted live reconciliation command is `/reconcile-fcr-governance-phase <40-char-current-main-sha> <founder_only|independent_review> <approval-reference>` on governance issue #418. The workflow accepts only the immutable founder account identity, rechecks exact current `main` before and after provider mutation, derives the review count from the allowed phase rather than accepting a caller-supplied count, uses the canonical provider factory under production GitHub App credentials, and requires phase-aware provider readback `READY`. The command grants provider-policy mutation for this bounded ruleset reconciliation only; it grants no merge, deployment, database, credential, billing, publication, or destructive-action authority.

The older `/reconcile-fcr-governance` path remains compatibility behavior for the historical one-or-more-review topology. The phase-aware command is the canonical current path because it names the intended constitutional phase explicitly.
...
The live GitHub human-review phase and FCR's internal deterministic evidence machinery are separate layers. `founder_only` removes the nonexistent outside-human reviewer requirement from the live repository ruleset; it does not erase exact-head machine checks, deterministic review evidence, founder-final binding, provider readback, or any other source/runtime authority contract that independently remains load-bearing.
