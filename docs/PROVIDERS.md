...
The constitutional target set for both FCR membranes is **main-only**. `scripts/audit-github-governance-preflight.mjs` accepts only the exact `refs/heads/main` target or `~DEFAULT_BRANCH` when that sentinel resolves to main, and reports `NOT_READY` if `~ALL` or any extra branch target is present. `src/providers/providerFactory.ts` independently rejects a canonical FCR ruleset mutation request unless `targetRefs` is exactly `['main']`, before delegation to the provider. Renamed/noncanonical FCR policies retain their separately reviewed flexibility. These source contracts do not repair or certify live provider state; only a trusted provider read after separately authorized reconciliation can establish that the live GitHub rulesets are main-only.
...
The canonical FCR ruleset cannot be disabled, demoted to evaluate mode, retargeted away from `main`, or broadened beyond `main` through the generic repository-administration route. A live provider rollback therefore means a separately authorized restoration to a previously proven **active** safe topology, with readback of both component identities; it does not mean turning governance off.
