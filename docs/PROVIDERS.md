...
For the governed `create_branch` execution path, `src/providers/authorizedRepositoryMutation.ts` is the final provider-neutral mutation membrane. It accepts only the canonical capability `github.repository.create-branch` and revalidates the FCR-issued envelope against the exact repository scope, idempotency key, proposal, branch arguments, current mission-state fingerprint, tool-call identity, expiry, and envelope integrity before `RepositoryProvider.createBranch(...)` is reachable. `src/founder-os-lab/authorityIssuance.ts` remains side-effect free: it does not read the wall clock or generate randomness, and deterministic defaults are derived from the server-observed proof timestamp and already-bound idempotency key. The HTTP runtime owns the fresh pre-mutation mission read and current-time check. This source contract governs the FCR `/execute` branch-creation path only; it does not prove that GitHub, GitLab, or another provider accepted a live mutation.
