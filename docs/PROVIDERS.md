...
For an **existing non-FCR named GitHub ruleset**, the repository-provider compatibility membrane now fails closed before provider mutation. It reads the current ruleset, rejects any requested target ref that remains explicitly excluded, rejects explicit bypass replacement through the narrow `RulesetConfig`, and refuses the remaining update until provider-supported atomic concurrency or an equivalent reconciliation contract can prove monotonic security across the write boundary. A pre-write read/merge alone is not sufficient because concurrent provider hardening could otherwise be overwritten by a stale snapshot. Brand-new ruleset creation and Founder Control Room's constitutional `main` ruleset continue through the existing canonical `GitHubProvider` behavior. This is source-side safety logic, not proof that any live provider ruleset currently has the intended topology.
