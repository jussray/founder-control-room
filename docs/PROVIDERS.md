...
For an **existing named non-FCR GitHub ruleset**, FCR now treats provider readback as the security floor before applying a narrower generic policy. The update path preserves unmanaged provider protections such as CodeQL, linear history, and Copilot review; preserves stronger pull-request parameters such as stale-review dismissal, latest-push approval, thread resolution, and a higher approval count; preserves retained status-check provider metadata such as `integration_id`; and preserves the existing bypass posture when the caller does not explicitly supply bypass actors. The caller may deliberately replace the required status-context list. Brand-new ruleset creation and FCR's constitutional two-membrane main policy keep their existing semantics. Repository source and tests prove this update behavior, but they do not prove any live GitHub ruleset has been mutated; provider mutation plus fresh readback remains separate authority.
