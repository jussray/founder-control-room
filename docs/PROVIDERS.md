...
For Chief governance specifically, `src/providers/trustedGithubGovernanceObserver.ts` and `src/providers/githubGovernanceReconciliation.ts` form a narrower evidence-only observation and reconciliation-planning boundary. Observation is pinned to `jussray/chief-ai-machine` and ruleset IDs `20818149` and `21261587`, uses FCR's repository-scoped GitHub App installation-token path rather than accepting a caller PAT/token, preserves required-check `integration_id` identity, requires complete bypass-actor and deployment readback, accepts provider-native bypass actors such as DeployKey entries whose `actor_id` is null, and fingerprints the complete provider observation. The founder-approved desired topology requires `Verify candidate ProofMode runtime with Playwright` on no-bypass exact-head ruleset `20818149` with GitHub Actions integration `15368`; that App binding is necessary but does not independently prove the workflow file or event that produced the check. The governance-boundary unconditional merge checks narrow to `Typecheck` and `Verify operational authority`, desired bypass actors are empty, and strict status freshness is required. `Cloudflare Production` and `proofmode-access-admin` are removed from pre-merge desired deployments but remain explicit post-merge/current-main production-proof obligations. The plan is bound to the complete observed provider fingerprints, declares unrelated provider security rules preservation requirements, and remains non-authorizing. Because GitHub's existing-ruleset update endpoint does not provide a documented atomic conditional-update precondition for the observed state, live mutation remains fail-closed until a separately reviewed drift-safe execution design is explicitly authorized and followed by fresh provider readback. Neither observation nor reconciliation planning grants ruleset mutation, Chief merge, FCR merge, deployment, production-proof, or execution authority.
