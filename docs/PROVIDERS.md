...
`src/providers/GitHubPrTruthReader.ts` is a narrow read-only provider adapter for the remote MCP `github_audit_pr` v0 surface. It may read only PR state, current-head check runs, workflows filtered to that head, reviews, and a bounded metadata-only commit comparison for the server-held `jussray/founder-control-room` repository. The audit re-reads PR state after those observations so a moving head becomes conflicted evidence, and passing CI for any other SHA is stale evidence rather than a pass. The adapter has no mutation methods, does not expose GitHub credentials, and its audit classification is evidence quality only, never merge approval.

For an **existing non-FCR named GitHub ruleset**, the repository-provider compatibility membrane reads the current provider ruleset before update and applies a monotonic security merge. `src/providers/SecurityPreservingGitHubProvider.ts` must never downgrade an existing ruleset's top-level enforcement, narrow its existing included branch scope, or discard existing excluded refs when applying a narrower caller request. Narrowing the requested required-status context list must not silently drop stronger provider-owned review settings, deletion/non-fast-forward protection, unmanaged rule types such as CodeQL, required linear history, or Copilot review, existing bypass posture, or provider metadata such as a retained status-check `integration_id`. Bypass intent is explicit: omitting `bypassActors` preserves the provider's existing bypass posture; supplying `bypassActors: []` intentionally clears all bypass actors; and a non-empty replacement fails closed because the narrow `RulesetConfig` shape cannot prove provider bypass mode until a separate provider-aware bypass-authority contract can prove that replacement is non-widening. Brand-new ruleset creation and Founder Control Room's constitutional `main` ruleset continue through the existing canonical `GitHubProvider` behavior. This is source-side preservation logic, not proof that any live provider ruleset currently has the intended topology.
