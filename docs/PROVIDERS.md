...
For an **existing non-FCR named GitHub ruleset**, the repository-provider compatibility membrane reads the current provider ruleset before update and applies a monotonic security merge. Narrowing the requested required-status context list must not silently drop stronger provider-owned review settings, deletion/non-fast-forward protection, unmanaged rule types such as CodeQL, required linear history, or Copilot review, existing bypass posture when the caller did not explicitly replace it, or provider metadata such as a retained status-check `integration_id`. Brand-new ruleset creation and Founder Control Room's constitutional `main` ruleset continue through the existing canonical `GitHubProvider` behavior. This is source-side preservation logic, not proof that any live provider ruleset currently has the intended topology.
