...
For a **non-FCR named GitHub ruleset**, the repository-provider compatibility membrane is deliberately fail-closed around provider concurrency. An existing named ruleset is observed only to detect exclusion and bypass conflicts; `SecurityPreservingGitHubProvider` does not PUT-update it because this repository has no reviewed atomic compare-and-swap/version precondition for the GitHub ruleset update endpoint. Existing non-FCR ruleset reconciliation therefore remains blocked until a separately reviewed concurrency-safe contract exists. A requested target that remains covered by an exact, wildcard, or broad existing exclusion fails closed, and explicit bypass replacement on an existing ruleset also fails closed because the narrow `RulesetConfig` cannot prove a non-widening provider bypass mode. When the named ruleset does **not** exist, the provider constructs the intended payload locally and issues one direct `createRepoRuleset` call; a concurrent same-name creation conflict is surfaced and must never fall back to PUT. Founder Control Room's constitutional ruleset path continues through canonical `GitHubProvider` behavior. This source membrane is not proof that any live provider ruleset currently has the intended topology.
