...
For an active ruleset protecting `jussray/founder-control-room` `main`, repository identity is constitutional and mutable project slugs are not. The normal GitHub path must preserve the FCR approval/check/force-push/deletion floor. Its bypass list must contain exactly one App actor whose numeric ID equals trusted `GITHUB_APP_ID`, and that App bypass is constrained to GitHub `pull_request` mode. Canonical founder-final integration must therefore use the provider-backed reviewed PR number plus the exact reviewed head SHA through the pull-request merge endpoint; the App is not granted an `always` bypass for direct protected-branch writes. Caller-supplied, missing, mismatched, additional, or broader bypass integration identities/modes fail closed. Provider readback must still prove the observed ruleset matches the requested policy; source validation alone is not live GitHub authority.

This preserves two separate membranes instead of weakening either one: ordinary GitHub merges remain subject to the native independent-approval ruleset, while the authenticated FCR founder-final path may cross that ruleset only through the trusted App, only through a reviewed pull request, and only after FCR has revalidated the exact base/head authority it already bound internally.
