...
For an **existing non-FCR named GitHub ruleset**, repository-owned mutation is currently fail-closed. The compatibility provider may read the current ruleset to classify the request, but it must reject any requested protected ref that remains explicitly excluded and must not issue a provider `PUT` for an existing ruleset until a separately reviewed reconciliation contract can prove concurrency safety. GitHub's ruleset update surface does not expose a repository-used atomic compare-and-swap/version precondition, so reconstructing a full update from an earlier read could overwrite provider hardening that lands between observation and mutation. Explicit bypass replacement remains separately fail-closed because the narrow `RulesetConfig` cannot represent provider bypass mode. Brand-new ruleset creation and Founder Control Room's constitutional `main` ruleset continue through the existing canonical `GitHubProvider` behavior. This is a source-side safety boundary, not proof that any live provider ruleset currently has the intended topology.
