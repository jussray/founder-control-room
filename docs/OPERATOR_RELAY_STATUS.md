...
- exact-provider adapters for OpenAI Responses, Anthropic Messages, and Perplexity Responses-compatible API calls exist only when both an explicit model and server-held key are configured;
...
- provider/model provenance is constructed server-side from configured provider identity plus a sanitized response ID, never from model-returned identity fields;
...
- a successful live OpenAI, Anthropic, or Perplexity provider response receipt from deployed FCR;
