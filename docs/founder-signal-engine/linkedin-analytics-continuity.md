...
A current decision observation must also declare a completed measurement window and `CURRENT` freshness. Historical, stale, superseded, invalidated, incomplete, malformed, or mixed-identity observations remain provenance but cannot be promoted into a current `SUPPORTED` verdict.
