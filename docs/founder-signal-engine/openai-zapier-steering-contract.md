...
A configured Zapier OpenAI connection may use the dedicated key without exposing the raw key to the acting agent. **Key possession alone is not a Zapier control surface; it is also not a Zapier administration surface.** A connector alone does not prove that the OpenAI action is authenticated.
