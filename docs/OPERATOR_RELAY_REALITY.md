...
- the peer relay contract exists for `codex`, `claude-code`, and `perplexity`;
...
- exact-provider adapters exist for OpenAI Responses, Anthropic Messages, and the Perplexity Responses-compatible API when server-held provider key plus explicit model are both configured;
...
The next gate is therefore live exact-head proof on the canonical runtime: deployed identity -> OAuth-bound founder request -> exact requested provider response -> evidence-bound return -> Playwright against the real path. If an authorized provider key/model or browser session is unavailable, preserve that one receipt as BLOCKED rather than substituting another provider or relabeling source tests as live evidence.