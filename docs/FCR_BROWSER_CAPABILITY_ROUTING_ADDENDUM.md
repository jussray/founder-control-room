...
If the connector surface exists but the live probe returns `Browser not connected`, classify the bridge as `BLOCKED_CONNECTOR_BRIDGE` in alignment with `.control-room/plugin-management.json`. Do not infer that the founder is logged out of the target provider, that a target tab is closed, that local setup is wrong, or that the provider page needs to be reset unless separate provider evidence proves that claim.
...
## Historical versus current capability truth

Current connector discovery and historical capability evidence are different time planes.

A fresh runtime observation may change the present state, but it must not delete, rewrite, or negate a verified historical receipt. If a connector was successfully used at time T1 and is absent from the current runtime at time T2, classify the two claims separately:

```text
historical = HISTORICALLY_VERIFIED
current = CURRENTLY_UNAVAILABLE
root_cause = UNKNOWN unless separately proven
```

Never transform current absence into `NEVER_EXISTED`. Never transform missing evidence into `NO`. Contradictory current signals, such as one surface reporting installed/enabled while another runtime surface cannot invoke the connector, classify as `CONNECTOR_STATE_CONFLICT` until reconciled.

Track material capability dimensions separately when evidence exists: catalog visibility, installation, authentication, callability, read capability, write capability, and successful execution. A positive state in one dimension does not graduate another dimension.
...
Historical receipts remain provenance and debugging evidence. Current action still requires fresh runtime discovery, current target identity, and the authority appropriate to the exact action.
...
Continuity fingerprints and proof cookies are non-secret correlation markers only. They never become browser tracking cookies, device fingerprints, session credentials, or authority tokens. Material movement in provider session, subject, authority, action arguments, runtime state, or evidence expires predecessor continuity for the current claim without erasing the predecessor receipt as historical evidence.
...
- erasing historical verified connector use because the current runtime no longer exposes the connector;
- converting contradictory connector signals into a false binary state instead of `CONNECTOR_STATE_CONFLICT`;
