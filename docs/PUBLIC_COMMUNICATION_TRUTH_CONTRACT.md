...
Prepared n8n execution reservations use the authoritative `approval_executions.started_at` value as their lease generation. Reservation and rearm must return the exact generation written by the database, and both pre-provider abort and successful receipt finalization must match execution ID, `pending` status, and that generation. A delayed worker from an older generation therefore cannot fail or finalize a rearmed worker's live reservation.
