...
On September 21, the same guard caught another legitimate peer advance: FCR's exact-head browser harness completed successfully while the federation check still pinned StoryEngine `39bed061b034793c69c262527d85c3c4b5617cce`, but live StoryEngine `main` had moved to `d3f75adecc99089989f47b953f7b737c0139a921` through PR #117. The local FCR browser success therefore remains historical rather than federation proof. Rebinding `.github/workflows/playwright.yml` to `d3f75adecc99089989f47b953f7b737c0139a921` selects a new evidence subject only; the successor FCR head must still rerun the complete peer runtime, directive, receipt, and browser witness before the integration can be called current.
