...
|---|---|---|
| Founder Control Room | One logical custom founder session | Express/Cloudflare backend + Supabase Auth + `founder_users` |
...
Production and preview use `__Host-fcr_session`; localhost uses `fcr_session` because Secure cookies cannot be set over plain HTTP. Both names represent one logical session. The cookie is HttpOnly, SameSite=Strict, Path=/, Secure outside local development, Priority=High, and capped at 30 days. Every authenticated request revalidates the Supabase user and the private founder allowlist. Browser mutations must pass the single exact-origin CSRF gate in `src/http/middleware/csrf.ts`.
...
Stripe Checkout may set cookies on Stripe domains after the user leaves the public hair storefront. Cloudflare may set edge-security cookies. Product code does not own, inspect, copy, or authorize from those values.
...
Every product repository also carries `.control-room/cookie-policy.json` and a repo-appropriate verification command. A proposed cookie without a manifest change is structurally unauthorized.
