...
A witness binds an evidence subject to an expected repository `main` SHA, verified evidence references, a verification timestamp, and an explicit expiry. Evaluation also requires a separate at-use `FreshnessObservation` containing the freshly observed current `main` SHA and observation time.
...
- Verification and expiry timestamps must be strict UTC ISO timestamps.
...
`VALID` means only that the supplied evidence is current against the supplied fresh observation. It never grants merge, deploy, publish, provider mutation, or other consequential authority. Callers must separately satisfy the applicable authority lease and provider/runtime gates.
