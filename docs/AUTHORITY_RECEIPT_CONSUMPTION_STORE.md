...
## Authority-role split

The v11.5 source migration fails closed by default. The generic Supabase `service_role` is explicitly denied every v11.5 kernel function, including authority initialization, authority transition, permit admission, outbox leasing, and outcome recording. This prevents an ordinary compromised application service from minting task authority, fabricating permit admission, or driving execution merely because it possesses a broad server credential.

Before production activation, a separate migration must bind narrowly held database identities with non-overlapping authority:
...
Those identities must not be generic application credentials and must be held only by the corresponding isolated runtime components. The source migration intentionally leaves all functions owner-only until that separation is proven.
...
- browser roles and generic `service_role` cannot read/mutate kernel tables or invoke kernel functions;
