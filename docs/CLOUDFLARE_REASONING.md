...
Provider truth is semantic while rollback authority remains identity-bound. If exactly one public Access application, managed or non-managed, covers only the exact `foundercontrolroom.org/*` public destination and independently proves `Bypass / Everyone`, readback may classify the front door as `already-public-bypass`. A behaviorally equivalent non-managed application is evidence only and never becomes a workflow-managed rollback target. Multiple matches, broader or multi-destination profiles, worker or preview-worker destinations, ambiguous policy semantics, managed-name drift, or any shape whose exact destination plus Everyone-bypass semantics cannot be independently verified fails closed for manual review. When manual review is required, public evidence may expose only bounded `destinationShape` and `destinationProfile` classifications, never raw provider payloads or application IDs. The recovery code does not rewrite the account-level `deny_unmatched_requests_exempted_zone_names` setting and does not alter existing all-workers protection.

A successful create is not production proof. It enters `mutated-needs-browser-proof`, then anonymous Playwright independently proves public stranger reachability and exact runtime SHA while separately verifying founder-only control-plane containment. Public reachability and founder protection are distinct assertions; neither can substitute for the other. If that post-apply proof fails, rollback may delete only the run-created managed application after reacquiring exactly one application with the same receipt-bound account, zone, application ID, managed name, and destination. Missing identity, ambiguity, or drift blocks rollback instead of widening deletion authority.
