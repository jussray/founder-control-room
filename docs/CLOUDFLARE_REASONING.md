...
Canonical `.github/workflows/deploy.yml` treats **Post-Deploy Reconciliation** as a load-bearing production gate. Reconciliation runs after smoke proof in the `production` environment and is not `continue-on-error`; `post-deploy-evidence` may run only after reconciliation succeeds. A failed reconciliation must therefore stop the durable evidence lane rather than being relabeled as a successful release. This sequencing is source authority only and does not prove that a production reconciliation run, provider state, or runtime outcome succeeded.
