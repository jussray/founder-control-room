...
## External governed MCP for compatible clients

The Control Room source defines a provider-neutral external connector boundary. A client implementation does not inherit authority merely because it can speak MCP.
...
Seven tools are read/preview-only. `fcr_relay_operator` is the one bounded external-provider side effect: it may relay research/propose/review/implementation work to exactly one addressed peer operator, but it carries zero repository/provider mutation authority and never creates founder approval or execution authority.

`github_audit_pr` is restricted to `jussray/founder-control-room`. It accepts a PR number and optional full expected head SHA, reads bounded GitHub PR/base/head/check/review/file evidence through the repository-scoped GitHub App path, omits raw patch bodies from its evidence model, and returns a non-authorizing continuity/evidence result. Incomplete, truncated, stale, conflicting, or moving provider evidence must not be promoted to complete current truth. It cannot approve, comment, dispatch, merge, deploy, publish, or mutate GitHub.
...
- register/allow the exact client IDs for connected MCP-compatible consoles;
- configure `FCR_REMOTE_MCP_*` and `CHIEF_AI_BASE_URL` without reusing provider/deploy credentials;
- prove the GitHub App has only the read permissions required by `github_audit_pr` and that the bounded reader fails closed when evidence is incomplete;
- prove the Chief URL/binding and FCR Worker SHA, then run the Attack Ten auth/scope/replay/header/evidence/client matrix;
- connect external clients only after provider evidence proves the resource metadata, OAuth flow, tools list, and calls from the deployed exact head.
...
Founder Control Room also serves a compatibility MCP gateway at `POST https://api.foundercontrolroom.org/mcp/read`. It uses the same bounded external catalog as the canonical OAuth lane, but the static-token compatibility identity cannot use the OAuth-only peer relay.
...
- `tools/list` is derived from the same explicit external catalog. There is no generic provider/tool escape hatch.
- `github_audit_pr` remains fixed to Founder Control Room and read-only even when invoked through the compatibility lane.
- `fcr_relay_operator` requires an OAuth-bound client identity and therefore fails closed on the static compatibility token.
...
The operator posture is deliberately asymmetric: external consoles may inspect bounded **active** portfolio truth while mutations still require the separate Ask-Founder / Founder Permission / execution-receipt path. A read token, OAuth project claim, messenger link, fingerprint, proof cookie, repository observation, or relay response grants no merge, deploy, provider, database, publication, billing, deletion, or arbitrary command authority.
