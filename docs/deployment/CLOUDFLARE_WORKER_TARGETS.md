...
A successful provider write is execution evidence, not browser outcome proof. Anonymous Playwright must independently verify no Cloudflare Access interception on the public apex/`www` doorway, that a random stranger reaches the FCR-owned founder sign-in surface without reaching the authenticated shell, that anonymous direct `api.foundercontrolroom.org/version` presents a Cloudflare Access boundary, and that same-origin `https://foundercontrolroom.org/version` proves the canonical `founder-control-room` Worker service identity and the exact approved SHA through the `FCR_API` Service Binding.

`HISTORICAL / SUPERSEDED (2026-09-25):` earlier wording required an anonymous stranger to receive the exact SHA directly from `api.foundercontrolroom.org/version`. That direct-API witness is superseded by [`docs/audits/2026-09-25-cloudflare-live-domain-access-boundary.md`](../audits/2026-09-25-cloudflare-live-domain-access-boundary.md); the direct API is a protected live-domain surface.
