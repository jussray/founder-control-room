...
Provider truth is semantic while rollback authority remains identity-bound. Exactly one existing public Access application, managed or non-managed, may be treated as `already-public-bypass` only when it covers only the exact `foundercontrolroom.org/*` destination and independently proves `Bypass / Everyone`. A behaviorally equivalent non-managed application never becomes a workflow-managed rollback target. Multiple matches, broader or multi-destination profiles, worker or preview-worker destinations, policy ambiguity, managed-name drift, or any shape whose exact destination plus Everyone-bypass semantics cannot be independently proven fails closed for manual review. Bounded public evidence may expose `destinationShape` and `destinationProfile` classifications for that review boundary, but not raw provider payloads or application IDs. The recovery still does not mutate DNS, Worker routes, the database, account-level `deny_unmatched_requests_exempted_zone_names`, unrelated Access applications, or existing all-workers protection.

A newly created public destination is only `mutated-needs-browser-proof`. Anonymous Playwright independently verifies public stranger reachability and exact runtime SHA while separately proving founder-only control-plane containment. Those are distinct assertions; neither can stand in for the other. If post-apply proof fails, rollback may delete only the run-created managed application after the receipt-bound account, zone, application ID, managed name, and exact destination are uniquely reacquired and still match. Ambiguity or drift blocks deletion rather than widening rollback authority.
