...
| Unbounded GitHub evidence | Returned/observed checks, commit statuses, raw workflows, reviews, and changed files are bounded at 100 records per collection. Review pagination uses at most the first and newest page needed to preserve recent state; no unbounded retry loop is added. |
