...
| A failing check/workflow exists beyond the 100-observation cap | Checks and workflow reads propagate provider total counts. Any truncated collection produces `evidence_collection_truncated` and cannot return `evidence_complete`. |
| New review state exists after the first 100 historical reviews | The bounded reader detects pagination and reads the newest review page. Omitted review history still marks the audit incomplete, while recent change requests/dismissals cannot be replaced by stale early approvals. |
...
| Unbounded GitHub evidence | Returned checks, workflows, reviews, and changed files are capped at 100 observations each. Review pagination uses at most the first and newest page needed to preserve recent state; no unbounded retry loop is added. |
