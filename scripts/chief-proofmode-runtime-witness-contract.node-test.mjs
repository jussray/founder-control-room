...
test('Access check and protected Playwright share one conflict-detecting canonical selector', () => {
  ...
  assert.match(workflow, /CHIEF_SELECTOR_REQUIRE_IDENTITY:\s*'true'/);
  assert.match(workflow, /Resolve canonical Chief Access selector without exposing identity/);
  assert.match(workflow, /node scripts\/resolve-chief-proofmode-access-selector\.mjs/);
  assert.doesNotMatch(workflow, /CHIEF_RUNTIME_ACCESS_CLIENT_ID:\s*\$\{\{[^\n]*\|\|/);
  assert.match(selector, /appendEnv\('CHIEF_CLOUDFLARE_ACCESS_CLIENT_ID'/);
  assert.match(selector, /appendEnv\('CHIEF_RUNTIME_ACCESS_CLIENT_ID'/);
  assert.match(selector, /conflicts with the generic fallback alias/);
