...
export function validateArchitectureKernel(kernel) {
  ...
  for (const capability of REQUIRED_CORE) {
    if (!uniqueCapabilities.has(capability)) errors.push(`missing core capability: ${capability}`);
  }

  const plugins = kernel.externalSystems;
  if (!plugins || typeof plugins !== 'object' || Array.isArray(plugins)) {
    errors.push('externalSystems policy is required');
  } else {
...
export async function verifyArchitectureKernel() {
  const raw = await readFile(KERNEL_URL, 'utf8');
